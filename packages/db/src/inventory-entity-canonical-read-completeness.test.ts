...
function belongsToInventoryEntityProjection(node: ts.Node): boolean {
  let current = node.parent;

  while (current) {
    if (
      ts.isPropertyAssignment(current)
      && propertyName(current.name) === "inventoryEntities"
    ) {
      return true;
    }
    ...
    if (
      ...
      return current.expression.expression.name.text === "inventoryEntity";
    }

    current = current.parent;
  }

  // Standalone selector constants cannot be attributed from syntax alone, so
  // keep them inside the guard unless a surrounding Prisma model proves otherwise.
  return true;
}

function nestedInventoryRelationshipReadsWithoutCanonicalPredicate(): UnguardedRead[] {
  const files = sourceRoots.flatMap((root) =>
    ts.sys.readDirectory(root, [".ts", ".tsx"], undefined, undefined),
  );
  const findings: UnguardedRead[] = [];
  ...
  for (const file of files) {
    ...
    const text = readFileSync(file, "utf8");
    if (!text.includes("fromRelationships") && !text.includes("toRelationships")) continue;
    const source = ts.createSourceFile(
      file,
      text,
      ts.ScriptTarget.Latest,
    ...
    const visit = (node: ts.Node): void => {
      if (ts.isPropertyAssignment(node)) {
        ...
        if (
          (name === "fromRelationships" || name === "toRelationships")
          && belongsToInventoryEntityProjection(node)
          && (
            node.initializer.kind === ts.SyntaxKind.TrueKeyword
            || ts.isObjectLiteralExpression(node.initializer)
          )
          ...
          const relationText = node.getText(source);
          if (!relationText.includes("INVENTORY_RELATIONSHIP_CANONICAL_WHERE")) {
            const position = source.getLineAndCharacterOfPosition(node.getStart(source));
            findings.push({
              file: relative(repoRoot, file).replaceAll("\\", "/"),
              line: position.line + 1,
...
describe("InventoryEntity canonical-read completeness", () => {
  ...
  it("keeps superseded relationship tombstones out of every default projection read", () => {
    const findings = [
      ...inventoryRelationshipReadsWithoutCanonicalPredicate(),
      ...nestedInventoryRelationshipReadsWithoutCanonicalPredicate(),
    ];
    expect(
      findings,
      findings
