...
// ─── Backfill support (BET-5) ────────────────────────────────────────────────
/** Scroll ALL points of a collection WITH vectors — used once to copy Qdrant → pgvector. */
export async function scrollAllPoints(
  collection: string,
): Promise<Array<{ id: number; vector: number[]; payload: Record<string, unknown> }>> {
  const out: Array<{ id: number; vector: number[]; payload: Record<string, unknown> }> = [];
  let offset: unknown = undefined;
  for (;;) {
    const body: Record<string, unknown> = { limit: 200, with_vector: true, with_payload: true };
    if (offset !== undefined && offset !== null) body["offset"] = offset;
    const res = (await qdrantFetch(`/collections/${collection}/points/scroll`, {
      method: "POST",
      body,
    })) as { result?: { points?: Array<{ id: number; vector: number[]; payload: Record<string, unknown> }>; next_page_offset?: unknown } };
