...
Every connect, disconnect, health, refresh, sync, and callback attempt maps to `IntegrationToolCallLog`. The record contains connector and actor identity, operation, a canonical hash of adapter-redacted input, stable response class, result count, duration, and a typed safe error. It never stores raw arguments, credentials, tokens, signatures, or callback payloads. Anonymous callbacks use the external-webhook actor.
