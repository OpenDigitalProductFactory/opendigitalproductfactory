...
**Prefer OAuth; a PAT is the fallback (BI-8E1D0D4D).** A client with no pinned `Authorization` header discovers the authorization server and authorizes once, then refreshes silently — there is nothing to rotate and nothing to expire mid-session. `.mcp.json` in this repo carries **no** `headers.Authorization` for exactly that reason: setting one *disables the OAuth fallback*, which is what kept this install on the expiring path. Only pin a header for a caller that genuinely cannot do OAuth, and expect to rotate it by hand forever.
