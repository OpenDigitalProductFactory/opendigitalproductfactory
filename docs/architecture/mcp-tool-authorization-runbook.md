...
**The URL scheme decides which credential path the client config carries (BI-46B636B0).** The client runs OAuth only over **https**; on a plain-http endpoint the authorization server is unreachable from the client however healthy it is server-side, and a pinned `headers.Authorization` disables OAuth. The two are therefore exclusive: an **https** endpoint gets no header and the client authorizes itself; an **http** endpoint (the local default, `http://127.0.0.1:3000`) keeps the `${DPF_MCP_BEARER_TOKEN}` header reference, which is its only credential path. One predicate — `mcpClientBearerHeaderRequired` in `@dpf/integration-shared/mcp-client-credential-policy` — is consulted by both writers of the config (the portal's setup snippets and the toolchain bootstrap planner), and the SessionStart MCP health hook warns at session start when a config cannot authenticate by any path. To move an install onto OAuth, serve it over https (`docker-compose.tls.yml`) and point `.mcp.json` at that URL; the bootstrap then drops the header.

**Prefer OAuth; a PAT is the fallback (BI-8E1D0D4D).** A client with no pinned `Authorization` header discovers the authorization server and authorizes once, then refreshes silently — there is nothing to rotate and nothing to expire mid-session. Setting a header *disables the OAuth fallback*, so it belongs only where OAuth cannot run — which, per the rule above, is every plain-http endpoint. Over https, pin no header; over http, expect to rotate the PAT by hand until the install is served over TLS.
...
3. Restart the desktop client. On first connection it discovers the authorization server, registers itself (RFC 7591), and opens a browser consent page on the signed-in portal; approve once. From then on the access token refreshes silently. `Admin > Platform Development` lists the client beside any remaining PATs.
