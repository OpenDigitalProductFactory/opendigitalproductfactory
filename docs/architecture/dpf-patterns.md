...
**Execution contract.** Cheap guards bypass admission. Heavy commands fail closed when host memory cannot be measured, preserve a host reserve plus the resident inference reserve, and serialize inference. A denied invocation records bounded queue diagnostics and exits with code 75; it does not leave the CALLING Node process blocked. The one process it may leave behind is the detached resumer below, which registers in the local queue observer registry and is reaped by it. The caller may retry, or hand the claim to the detached resumer that exits alongside it (`scripts/local-ci-durable-wait-resumer.mjs`), which re-claims until the lease is admitted. It is backstage in every sense: the resumer and each gate run it starts are spawned with `windowsHide`, because a detached process owns no console and on Windows its console child otherwise opens a visible, focus-stealing terminal window on every re-claim (BI-69178E02). Because the gate client and its resumer run from the branch being gated, a client defect fixed on main is retired from un-rebased branches by the admission server, not by each host: every gate claim reports `gateClientRevision`, and the server refuses a claim below its floor (`apps/web/lib/nonprod/gate-client-revision.ts`) with `gate_client_upgrade_required`. That answer is not a verdict on the diff, and it ends a stale resumer because it is neither queued nor transient. The queued report names the owner: `resumeOwner: "detached-resumer"` means the caller must NOT poll, `resumeOwner: "caller"` means it must. A lease event alone never resumes anything - the capacity wake writes a TaskRun projection, and a projection is not a process (BI-D35B85BF). Only the process identified by the lease's PID plus operating-system process-start identity is supervised. Stray processes are evidence, never kill targets.
