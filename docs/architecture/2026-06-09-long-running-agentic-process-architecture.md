...
| HITL gate as a product surface | **`DecisionPerspectiveGatePanel`** + `ActionBanner` capture escalate/defer outcomes; a `decline` outcome (BI-2107B5D2) renders as a settled answer rather than a pending capture | [`DecisionPerspectiveGatePanel.tsx`](../../apps/web/components/build/DecisionPerspectiveGatePanel.tsx) |
