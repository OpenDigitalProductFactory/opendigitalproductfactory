...
| Deferred tool exposure on external CLI path | Implemented | `apps/web/lib/mcp/tool-tier.ts`, `apps/web/lib/mcp/load-tools.ts`, `apps/web/lib/tak/tool-intent.ts` | Core tier is the **default for non-Claude-Code clients**; `load_tools` expands the authorized per-token surface by exact name or natural-language intent, append-not-swap, with `list_changed` plus deterministic re-list fallback. Client-host consumption is verified separately from protocol conformance. |
