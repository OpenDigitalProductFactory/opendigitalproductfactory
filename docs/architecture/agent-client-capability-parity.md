...
- **Lean tool surface:** external CLI core-tier discovery (~27 tools) is now the **default for non-Claude-Code clients** (Claude Code keeps full; opt back with `?tier=full`); see the MCP tool-authorization runbook.
