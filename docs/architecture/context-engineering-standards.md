...
- **P4 — Just-in-time over front-loading.** Pass handles (paths, IDs, queries); retrieve bodies on demand; structure is signal. DPF: corpus slug-prefix retrieval, skill *summaries* not bodies, L3/L4 deferred to tools. External-CLI `tools/list` **defaults non-Claude-Code clients to a lean core tier** (Claude Code keeps the full surface because it defers the catalogue client-side; Codex/Grok/customer/unknown clients get core by default and opt back in with `/api/mcp/v1?tier=full`; `apps/web/lib/mcp/tool-tier.ts`), so a client with no client-side deferral stops paying the full tool-definition tax up front. The core tier is not only read/discovery: it also carries the gated live-delivery tools external agents need after merge (`request_self_upgrade`, quiescence/queue reads, and runtime-verification evidence). Model-driven deferral is **shipped**: the always-listed `load_tools` meta-tool accepts exact names or natural-language capability intent, appends authorized matches to the per-token session, emits `notifications/tools/list_changed` over SSE, and tells notification-blind clients to re-fetch `tools/list`; the core remains the floor. **Operational / system-of-record data (orders, invoices, customer accounts, quotes, token-usage and tool-execution ledgers, build activity) is fetched LIVE via Prisma and NEVER vectorized** — memorizing a live record means reasoning over a stale snapshot. Only durable KNOWLEDGE (WWMD/WWWD corpus, documents, conversation/platform/capability memory) is embedded. **[ENFORCED]** `apps/web/lib/inference/jit-retrieval-discipline.test.ts` freezes the set of modules that may call `generateEmbedding` (the single embedding choke point). *[REVIEW]* for the handle/deferral aspects.
