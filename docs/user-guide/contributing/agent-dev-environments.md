...
2. **Mints and persists a legacy DPF MCP token** if one isn't present (a client that speaks the browser flow no longer needs this — see [How a client authenticates](#how-a-client-authenticates-all-clients)) (issued inside the portal container, persisted to `~/.dpf/agent-toolchain.env` and your shell profile, never logged). Default scope is `write` so the agent can use side-effecting MCP tools (backlog, evidence, Build Studio handoff). A token that is present but **rejected by the portal (HTTP 401 — expired after its 90-day TTL, revoked, or issued for another install)** is re-minted on the next run too; an unreachable portal never replaces a present token. On macOS it also injects the token via `launchctl` so GUI-launched apps — which don't read your shell profile — still pick it up.
...
The readiness record is written to its own sidecar, `~/.dpf/agent-toolchain-state.json`, with a plain atomic write. It deliberately does **not** live inside `install-state.json`: the self-upgrade binds that file byte-for-byte in a signed handoff, and this bootstrap runs on every client session start — one landing during an upgrade's quiescence drain fenced the upgrade with `install_state_envelope_state_changed` (BI-95DF1BFC). Starting a client session while an upgrade is draining is therefore safe. A failed sidecar write still stops the bootstrap with an explicit error instead of reporting a misleading ready banner.
