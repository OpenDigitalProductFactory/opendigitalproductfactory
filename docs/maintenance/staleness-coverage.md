...
**205 of 698 published doc pages (29.4%) carry a doc-impact edge.**
The other **493** (70.6%) carry none, and are invisible to
...
Of those 493, **478 reference no repo file anywhere in their text** — no link, no
...
The genuinely recoverable set is **15** page(s) — those that cite a repo file but
whose citation the current derivations do not turn into an edge.

The timestamp detector (`build-docs-staleness.mjs`, BI-AA5DFEA2) currently reports **164** stale
...
| `runbooks` | 7 | 3 | 42.9% |
| `design` | 4 | 2 | 50% |
| `operations` | 22 | 12 | 54.5% |
| `architecture` | 104 | 64 | 61.5% |
| `security` | 9 | 6 | 66.7% |
...
1. **Widening is close to exhausted.** Of the pages any code-edge derivation could
   reach, 205 of 220 (93.2%) now carry an edge. The headline 29.4% understates
