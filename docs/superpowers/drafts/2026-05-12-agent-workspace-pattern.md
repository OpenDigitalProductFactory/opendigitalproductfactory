...
**Status:** draft. Observations are grounded where marked; open decisions remain explicit. No code changes are proposed in this draft. Companion work: [document-management capability kickoff](2026-05-12-document-management-capability-kickoff.md).
...
**Open decision:** whether the Markdown operator contract becomes canonical and the Admin > Prompts surface becomes an editor for that file, or whether DB prompt templates remain the editable source of truth.

Proposed default: repo Markdown is canonical for operator contracts; DB prompt templates become small runtime headers.
...
2. Strict or loose audit boundary?
3. Is repo Markdown canonical for operator contracts, with DB prompts as per-turn headers?
4. Are sub-agents encouraged inside workspaces?
5. What is the supported sandbox-to-portal MCP transport path?
6. Do document workspaces wait for the document-management capability?
7. Should `tools/list` gain workspace/phase filtering beyond token scopes?
8. Should MCP token issuance generate client-specific config snippets, including `type: "http"` where required?
...
- Fix the generated MCP config snippets.
- Define the internal sandbox-to-portal transport rule.
- Re-run the authenticated CLI probe.
