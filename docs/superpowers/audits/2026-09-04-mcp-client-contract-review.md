...
That premise does not establish equivalent client behaviour. [Claude Code documents](https://code.claude.com/docs/en/mcp) disk persistence above its default output threshold and a per-tool `_meta["anthropic/maxResultSizeChars"]` hint, capped at 500,000 characters. [Codex documents](https://learn.chatgpt.com/docs/config-file/config-reference) `mcp_servers.<id>.tools.<tool>.output_token_limit` as an output truncation budget; that setting does not establish lossless persistence. DPF's preview marker discards structured data before either client receives it. BI-3CE72645 should repair the bounded page/read contract and return typed summaries with supported continuation. Client-specific thresholds are optional measured adapter choices, not a substitute for this server guarantee; verify any persistence claim by reading the saved result back.

### 3. The efficiency report silently excludes newer activity — high priority

Requested `analyze_mcp_call_efficiency({windowHours:168,notify:false,dispatchAiOps:false})` at 23:51 UTC. It returned 5,000 calls with observed boundaries August 28 23:51 through August 29 08:20, successRate=0.946 and ledgerSufficiency.usable=true. A separate one-hour query returned 94 calls from September 4, proving newer records exist. The Claude Code re-run at 00:18 UTC on September 5 returned boundaries August 29 00:18 through 08:30. Both seven-day probes cover roughly their first eight hours; they do not establish a fixed coverage duration for other windows.

`operate/mcp-call-efficiency/report.ts:66` orders ascending and takes the caller's limit (default 5,000, loader maximum 20,000); the MCP tool schema does not expose that limit at all. The analyzer reports observed event boundaries, not requested-window coverage. The seven-day result cannot support a seven-day health claim. The same loader feeds `queue/functions/mcp-call-efficiency-scan.ts` (06:15 UTC daily, `windowHours:24`, notification and AI Ops dispatch enabled). A daily window exceeding 5,000 eligible rows therefore omits its later rows; the number of hours omitted depends on volume and was not measured here. The repeated lease-call findings from August 29 are investigation leads within the sampled range, not verified current findings. Whether scheduled notifications or dispatches actually occurred requires their own execution evidence.
...
Preserve ToolExecution as the ledger. BI-4BB68EB6 should make requested range, observed range, population count and completeness explicit, then aggregate with bounded memory. Partial scans may issue labelled coverage diagnostics with observed bounds and continuation; they must not file corrective backlog items or dispatch AI Ops. tools/list and pre-auth failures remain unlogged gaps already acknowledged in the canonical efficiency design. Claude Code 2.1.222 fixed `/usage` MCP attribution so a server's token share reflects only requests that consumed its results; that is an independent, client-side measurement of DPF's context cost worth recording alongside the ledger.
