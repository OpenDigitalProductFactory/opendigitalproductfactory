...
- A bound read whose result has `hasMore=true` is partial evidence. It satisfies “reader ran” but not “evidence complete.”
...
- Evidence is complete only when one exact-bound ordered attempt starts at the beginning and ends with `hasMore=false`, or deterministic hydration produces that sequence.
...
No schema or receipt type changes. Persisted `ToolExecution` rows remain the audit source; source bytes stay bounded and redacted under existing policy.
...
4. Prove a 311-line fixture reaches the final page before its writer; incomplete traversal creates no receipt.
5. Run affected suites, typecheck, preflight, exact-tree semantic review, exact-tree local CI, DCO PR, and protected merge.
...
- Cursor gaps, identity conflict, and budget exhaustion fail closed without a writer execution or receipt.
