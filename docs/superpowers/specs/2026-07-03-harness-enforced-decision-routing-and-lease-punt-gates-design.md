...
| **Grok** | interactive ask-tool | `Bash` | **Gate B — NOT LIVE (live-probed §11, BI-883FC2FC).** Manifest `../` path bug (fixed here) had loaded zero components; even with hooks loaded, Grok does not execute the Claude `${CLAUDE_PLUGIN_ROOT}` PreToolUse *blocking* command-hook contract → follow-up. Gate A residual → BI-B22DE548 |
