...
**P5. Few, consolidated, unambiguous tools.** "If a human can't say which tool to use, neither can the agent." Namespace; onboarding-doc descriptions; actionable errors. → Fix the 12 `search_*` / 7 `record_*_evidence` overlaps; strip provenance from descriptions; keep exposed sets under the 15-tool local cliff.
...
- ⚠️ **Description bloat + convergence debt (G4/P5).** ~88KB of descriptions; ~a dozen leak `BI-xxxxxxxx`, `Phase 4b/7`, and file paths the model doesn't need. 12 overlapping `search_*`, a 7-way `record_*_evidence` family, `propose_decomposition` vs `propose_build_decomposition`. Every leaked token is paid on every `tools/list`; overlaps degrade selection.
...
- 🟡 **Memory salience across compaction (P11).** Facts enter via arbitration once; unlike `withPlanReminder`, there's no re-injection guarantee if the window slides on a long chat. (`2026-05-14-coworker-memory-shape-contracts` is the place to extend.)
...
**R7 — Registry convergence: faceted search + evidence consolidation (P5).** Collapse the 12 `search_*` into one faceted `search` (facet enum) and the 7-way `record_*_evidence` into one `record_evidence` (kind enum), preserving grant boundaries. Continue the started decomposition of the 16K switch into `*-mcp-handlers.ts`. Reduces both maintenance debt and token surface. *Larger, schedule deliberately.* Handle: *proposed under EP-ARCH-CONVERGENCE*.
...
**R9 — Memory salience across compaction (P11).** Generalize the `withPlanReminder` "survive the window" pattern to governed-memory facts on long coworker chats (re-assert top facts post-slide). Extend `2026-05-14-coworker-memory-shape-contracts`. Handle: *proposed BI — memory-salience-reinjection*.
