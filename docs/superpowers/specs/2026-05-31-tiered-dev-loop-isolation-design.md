...
| **BI-166C59F3** | Per-worktree script + pre-push hook | `pnpm run pregate` claims `local-integration-ci` lease, runs the command supplied by the governed local-CI runner (`DPF_LOCAL_CI_COMMAND` during the transitional script phase), and records `gatePassed` from the real command exit status. Stub passes are refused by default. Pre-push hook refuses ungated pushes (with `DPF_SKIP_PREPUSH=1` bypass). |
