...
- **Shipped (the design-review sub-case — the ideate analog):** the *first* phase had the same gap one step earlier — a FEATURE build whose designDoc failed review ("fails to address critical security / input-validation / audit-logging" — the local model under-weighting cross-cutting concerns) sat in `ideate` re-failing the same review every resume. `dispatchDesignReviewFixLoop` (`ideate-on-approval.ts`) regenerates the designDoc with the reviewer's issues fed back (`dispatchIdeateForApprovedBuild` gains `priorReviewFeedback`+`forceRegenerate`), re-reviews, repeats up to `DESIGN_FIX_MAX_ROUNDS`, then escalates. Scoped to feature builds — a `kind=fix` build's "Incomplete fix diagnosis" is a missing `fixContext` a regen can't fill, so those escalate directly (fixContext population is a separate item). The resume path runs it on a failed prior review. *Also fixed in the local-tuning pass:* Ideate ran on cloud chatgpt on opencode installs (provider fallthrough), now local; and the resume path **regenerates** a failed plan instead of re-reviewing it forever (the plan analog of this resume fix).
