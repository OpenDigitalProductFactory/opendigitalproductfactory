...
**Rationale:** Per the [`dpf_as_integration_conduit`](../../../../../DPF/docs/founder-kernel/wiki/principles/) standing rule, enterprise integrations work via the customer's own accounts and credentials. The "financing offered at invoice" pattern in §4.1 and §9.2 is correct in intent but must be implemented as: (a) detect the threshold, (b) generate the financing application link or rebate form the operator/customer signs in to themselves, (c) capture the resulting state back on the `WorkItem`. DPF never sits in the partner contract.
