...
- **Layer 1 shipped-standard:** 87 industry archetypes across 19 families; 112-page founder kernel; built-in compliance catalog with a *generic applicability evaluator* (new regulation = data seed, not code); COA fragments per ledger model; ~21 coworker roles; APQC-style capability tree; build-process right-sizing matrix; device-fingerprint catalog. A fresh install boots populated where ServiceNow ships blank tables.
