...
4. **Extend BI-FD796419 — "Where we need you" plain stop layer (Band 4).** Add the plain *what-it-means* sentence per stop + replace the review-failure chat-punt with a guided in-place recovery (two-choice + "Try to fix"), countering the 70%-then-cliff anti-pattern. *Relates to / extends the existing item — not duplicated.*
