...
6. **BI-1093AF1C - InventoryEntity <-> FixedAsset bridge** - define optional serial/assetTag/catalogIdentity matching and origin trace fields needed before any finance-facing HAM automation. **Read-model reconciliation LANDED** (same read-model shape as the CCI bridge; kernel DI-56B99A37FA40). Unblocked by the UniFi serial-collector prerequisite — `FixedAsset` carries no manufacturer/model, so `packages/db/src/inventory-asset-bridge.ts` matches on **serial only**, fail-closed (no serial on either side → no match — a false capitalization link is worse than none). `correlateDiscoveryToAssets` reconciles the org's own estate (`organization:internal`) into capitalized-and-discovered / on-books-not-discovered (retired/missing) / serial-bearing-but-un-capitalized, surfaced read-only via `getAssetRegisterReconciliation()` (`manage_finance`-gated) for the finance/assets surface. No authority move, no persistence. Follow-ups: **SNMP ENTITY-MIB serial capture LANDED** (`discovery-collectors/snmp.ts` walks `entPhysicalSerialNum` 1.3.6.1.2.1.47.1.1.1.1.11 → first non-empty = chassis serial → `attributes.serialNumber`), broadening serial coverage from UniFi to any SNMP device (switches/routers/servers); host DMI/SMBIOS serial and a confirmed-link + origin-trace persistence remain.
