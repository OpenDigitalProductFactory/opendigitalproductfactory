...
| Field | Value |
| ----- | ----- |
| Status | Draft - chief architect review applied 2026-06-20; responder WWMD-consult mechanism + unified decision front door (5.7) + trust dial / autopilot maturation (§14, operator doctrine) completed 2026-06-20. Root-cause prevention (planner kernel-lens, BI-C2CB3073) shipped #2195. **Slice 1 (classifier + projection guard) shipped #2265.** **Slice 4 first cut (surface convergence): the Attention lens now surfaces in /ops; /admin/issue-reports reframed as evidence.** Receiving loop (BI-0ACD9AB2) is HITL-first per §14. **Slice 2 shipped: operator-triggered WWMD consult (dial-low) on each /ops escalation card, AND an autonomous advisory sweep (dial-up) in the 15-min cron that pre-consults each held escalation with the bootstrap-owner principal (`resolveScheduledOwnerUserId`) and stores the recommendation, shown without a click.** Both ADVISORY — no autonomous build disposition. Full autonomous disposition (execute authority) is the remaining dial-up. |
| Date | 2026-06-20 |
