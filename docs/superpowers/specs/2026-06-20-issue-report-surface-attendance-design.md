...
| Field | Value |
| ----- | ----- |
| Status | **§5.3 / §14 WWMD-consult SUPERSEDED 2026-06-23** — `principle_decide` is degenerate on the escalation resume/defer axis (every option scores composite 0.000); the consult is replaced by honest blocker context + a conservative auto-resolve sweep. See [2026-06-23 escalation surface honest-context + auto-resolve design](2026-06-23-escalation-surface-honest-context-and-auto-resolve-design.md). The classifier + projection-guard + attention-lens convergence below remain in force. ⸻ Draft - chief architect review applied 2026-06-20; responder WWMD-consult mechanism + unified decision front door (5.7) + trust dial / autopilot maturation (§14, operator doctrine) completed 2026-06-20. Root-cause prevention (planner kernel-lens, BI-C2CB3073) shipped #2195. **Slice 1 (classifier + projection guard) shipped #2265.** **Slice 4 first cut (surface convergence): the Attention lens now surfaces in /ops; /admin/issue-reports reframed as evidence.** Receiving loop (BI-0ACD9AB2) is HITL-first per §14. **Slice 2 shipped: operator-triggered WWMD consult (dial-low) on each /ops escalation card, AND an autonomous advisory sweep (dial-up) in the 15-min cron that pre-consults each held escalation with the bootstrap-owner principal (`resolveScheduledOwnerUserId`) and stores the recommendation, shown without a click.** Both ADVISORY — no autonomous build disposition. Full autonomous disposition (execute authority) is the remaining dial-up. |
| Date | 2026-06-20 |
...
**Dial-low first cut shipped (§14):** an OPERATOR-triggered WWMD consult on each escalation in the `/ops` attention lens — `consultEscalation` action → `buildEscalationDecisionRequest` → `evaluateBuildStudioDecision`, run with the operator's own principal so it needs no system-responder identity and clears the system-user-sentinel guard. The kernel returns resume / defer / escalate-human with confidence + reasoning (operator-sanitized); the human decides, no autonomous side effects. **Dial-up shipped too:** an autonomous advisory SWEEP in the 15-min `issue-report-triage` cron pre-consults each held escalation — `runEscalationResponder` resolves a REAL principal via `resolveScheduledOwnerUserId` (the bootstrap superuser, never the banned `"system"` sentinel), runs `evaluateBuildStudioDecision`, and stores the recommendation on `PlatformIssueReport.responderDecision`/`responderDecidedAt` so the `/ops` lens shows it without a click (`EscalationConsult initial`). Still ADVISORY — it records a recommendation; it does NOT autonomously resume/kill the build. Full autonomous DISPOSITION (execute authority — resuming/killing builds) is the remaining dial-up beyond this, and is the genuinely careful TAK design.
