...
- **(a-iii) Coverage-model gaps found by running the guard against a real cut (2026-08-01).** Three, all fixed, all of the same family — *the check was measuring the wrong thing, and the cheapest way to silence it was to damage the doctrine*:

  1. **Raw link-target comparison.** Relocating a rule one directory down rewrites its anchor to `../founder-kernel/…`; the guard read all six as deleted. Anchors now resolve to repo-relative paths.
  2. **Anchor-only duplicate detection.** One kernel principle legitimately backs several distinct rules — `single-source-of-truth` backs both "each rule in exactly one place" and "coworker capability filtering is single-source". All four flagged "duplicates" in the post-Phase-1 plane were of that kind and **zero were real**; deleting anchors to clear them would have cut discoverability to satisfy a mis-specified check. Detection now requires the *labels* to restate each other, scored by Jaccard (a shared-over-shorter ratio let a three-word label like "Single source of truth." trip on two shared words).
  3. **Rules pinned by code, not by an anchor.** `apps/web/lib/decision/dimension-catalog.test.ts` parses `AGENTS.md` and cross-checks the `principle_decide` cost-axis names against the code registry. Phase 1 relocated that sign rule; the anchor check stayed green and CI caught it. The guard cannot infer such dependencies, so `manifest.contentAssertions` now registers them and any unregistered reader is reported — the set cannot grow silently. Detection is deliberately narrow (a read call whose own argument names the doc), because matching "mentions AGENTS.md" flags ~20 files that only cite it in a comment.

  The through-line for Phase 2/3: **a guard that fires on the operation it exists to protect is worse than no guard**, because `--update` is always the cheapest response.
