...
PR acceptance is tiered rather than monolithic. The required Production Build owns full candidate compilation; the self-upgrade PR check builds only the immutable promoter and proves readiness success, invalid-state refusal before quiescence, signed migration, and exact rollback contracts. The full portal-to-portal N-1 runner is restricted to scheduled/manual execution on a canonical runtime substrate capable of completing Docker image export. A PR evidence artifact declares `scope: bounded-pr-contract` and never misrepresents that result as a full N-1 run. WWMD decision `DI-FA01D947B1FA` selected this tiering with high confidence and a 2.499 margin after repeated hosted-runner termination demonstrated that the monolithic PR check was not executable evidence.
