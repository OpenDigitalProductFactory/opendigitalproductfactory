...
- **Artifact janitor — scheduled in OBSERVE / DRY-RUN mode (BI-DBF3F426, 2026-07-16).** `scripts/runtime-artifact-janitor.mjs` (reaps orphaned `dpf-local-integration-*-build` images + stray/foreign compose projects) now has a scheduled home: `apps/web/lib/queue/functions/runtime-artifact-janitor.ts`, daily `20 5 * * *`. It runs the janitor's *detection only* (`--json`, never `--apply`), **logs what it WOULD reap** (what / why-classified-orphan / age) and records a summary — **it deletes nothing**. Gated by `DPF_RUNTIME_ARTIFACT_JANITOR_ENABLED` (default OFF). Under `destructive-actions-require-explicit-go`, actual `--apply` reaping stays a separate founder-gated operator action; there is intentionally no code path from the schedule to `--apply`. The orphan criteria the founder reviews before enabling apply: a build image is a candidate once it matches `^dpf-local-integration-[a-z0-9-]+-build$` **and** is idle ≥ staleness-days (default 7); a compose project is a candidate only when it is not the root `dpf` project, has **no** live worktree backing it, **and** its newest container is idle ≥ staleness-days.
