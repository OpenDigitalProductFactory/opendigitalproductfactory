...
| ------------ | --------------------------- | ------------- |
| **QuickBooks financial truth** | `Invoice` model already carries the job rails: `sourceType`/`sourceId` (→ `WorkItem.itemId`), `erpSyncStatus` (`pending\|synced\|failed\|blocked`), `erpRefId` (QB invoice id), and `@@index([sourceType, sourceId])`. QB read client + OAuth/token storage (`IntegrationCredential`) live in `apps/web/lib/integrate/quickbooks/`. Governed-write pattern exists (`AgentActionProposal` + `approveProposal()`). | QB **write** client (`createQuickBooksInvoice`), the job-complete→DPF-invoice action (`sourceType="field-service-job"`), and the approval-gated sync proposal. No new schema. |
