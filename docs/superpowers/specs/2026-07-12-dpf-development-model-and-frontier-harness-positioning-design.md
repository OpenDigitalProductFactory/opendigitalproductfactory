...
4. **Gates.** ~20 distinct gates (full inventory: Build Studio process map, §3 of the research record). Nearly all are automated: dual-AI design/plan review with oscillation detection, the WWMD plan→build kernel gate (fail-closed), releasable-diff, typecheck, pre-PR security/destructive-SQL/architecture gates, CI required checks, Spec/Plan/Doc + UX-Fit + Seed-Fit attestation gates, watchdogs and reapers for unattended operation. **One intentional human gate remains in-pipeline: the "Ship to GitHub" click.** *(Amended 2026-09-25, founder decision: Build Studio now opens the pull request itself once review passes — BI-D80F2EA0. The in-pipeline click is gone; required checks, the merge queue and human review still decide what merges.)* Post-PR, the human residue is whatever escalations the decision ladder can't clear.
