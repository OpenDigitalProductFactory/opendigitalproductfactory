...
> **Implemented (BI-OPT-CONVERGENCE-UTILS, the slug + CSV parts of F-E).** Two shared utilities now exist under `apps/web/lib/shared/` (both pure, no Prisma/React, barrel-exported via `lib/shared/index.ts`):
> - **`slugify.ts`** — canonical `slugify(s)` = `s.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")`. Migrated **18** verbatim kebab twins across `apps/web` (business-model, project-events — the live `(^-|-$)` drift, mcp-tools, branding, promotions, work-capsules fallback, contribution-pipeline, identity-privacy, business-capabilities, setup-entities, ingest-500-agents, seed-market-offer, roster-import, research-capture, mcp-catalog-sync, build-studio-branch-badge, SetupWizard, archetypes route). Parity proven: the three boundary-strip variants (`^-+|-+$`, `^-|-$`, `(^-|-$)`) are output-identical because `[^a-z0-9]+` already collapses runs, and `.trim()` is a no-op. Length caps / `||` fallbacks / a `.git` strip are composed around the shared call, never folded in.
> - **`csv.ts`** — server-safe RFC-4180 `toCsv(rows, columns?)` (quote `["\r\n,]`, escape `"→""`, join `\r\n`) + `escapeCsvField`. Backs `grid-csv.ts` (byte-identical — it already used CRLF) and `actions/reports.ts` (which the finance route reuses). The reports serializer's prior `\n` join + comma/quote-only escape was a latent bug (unquoted embedded newlines); converging it onto `toCsv` standardizes line endings to CRLF — the one intentional behavior change, with `reports.test.ts` updated to lock it.
> - **Deliberately NOT folded** (different output alphabet — would change output): the `packages/db` slug sites (`device-investigation`, `discovery-promotion`, `project-archetype-supply` — the lower layer cannot import from `apps/web`; sharing across both would need a second util in `@dpf/db`, out of this BI's scope); `slugifyReferenceModelName`; the snake_case `_` normalizers (`AddColumnButton`, `seed.ts`, `device-placement.test`); the `[^a-z0-9-]` storefront-slug (`storefront/settings/page.tsx`) and `[^a-z0-9._-]` GAID (`principal-linking`, `normalizeGaidLocalId`) normalizers; `work-capsules` `buildCapsuleSlug`'s diacritic (NFKD) main path; and the client report-kit `toCsv` (papaparse-backed, object-row shape — left as the client affordance). `e2e/helpers/reset.ts` carries a twin but is outside the BI's `apps/web`/`packages/db` scope.
