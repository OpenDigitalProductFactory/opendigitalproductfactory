...
**Status:** Phase 1 SHIPPED — core tier on `tools/list`, defaulted by proven client capability. Claude Code and Codex bootstrap with explicit `?tier=full` because their hosts index the authorized catalogue and attach tools lazily; generic/Grok/unknown callers receive `core`. User-Agent recognition is compatibility-only because current Codex HTTP requests omit it. Phase 2 (model-driven deferral) SHIPPED — `load_tools` meta-tool + per-token `McpToolSession` short-TTL store + `notifications/tools/list_changed`. `tools/list` returns the tier floor UNION the token's session-loaded tools UNION `load_tools`. See §4 and `apps/web/lib/mcp/tool-session-store.ts`.
