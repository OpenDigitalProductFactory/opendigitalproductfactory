...
| Founder network authority | Founder-hub master shared portfolio (installation display name is private configuration) |
...
3. **Reseller and Founder Hub installations** use the same relationship protocol with a different policy. Founder Hub is the commercial/channel hub for reseller relationships and the governance authority for the shared code base. Its master **shared** portfolio contains Founder Hub-owned work and demand that another party separately elects to share; it does not ingest reseller/customer backlogs through code contribution.
...
The architecture is a relationship graph rather than a fixed three-level tree. That supports the two Founder Hub development installations now, a reseller with many customers, a distributor with resellers, customers with more than one specialist partner, future community peers, and disconnected/offline operation without redesigning the transport.
...
> A non-technical operator can connect the right DPF installations in a few guided actions; each installation remains authoritative for its local work; approved shared intent flows reliably with provenance, privacy, and revocation; Founder Hub manages the reseller network and shared code governance; and code contributions reveal results rather than the contributor's backlog.
...
- A reseller can forward a sanitized demand envelope to Founder Hub while preserving the originating customer's chosen attribution or pseudonymity.
- Founder Hub counts one underlying demand signal once even if it traverses customer → reseller → founder.
...
- Giving Founder Hub or a reseller general administrative access to customer installations.
...
| --- | --- | --- | --- |
| `same-organization` | Founder Hub Mac ↔ Founder Hub Windows; future internal nodes | Shared platform demand, disposition, claims, release notices | High collaboration, still explicit pairing and field allow-list |
...
| `channel` | Customer ↔ reseller, reseller ↔ distributor, or reseller ↔ Founder Hub | Curated/pseudonymous demand upstream; catalog, disposition, and release notices downstream | Each party approves its own projection |
...
- It owns cluster decisions and shared-portfolio disposition only for Founder Hub-originated demand and demand envelopes another party explicitly shares through the demand channel.
- It does not own a customer's or reseller's local backlog status, priority, private rationale, or implementation commitments.
- A Hive Mind code contribution contains the submitted result and evidence, not the originating backlog record. Founder Hub may know the contributing organization/pseudonym and contribution purpose without seeing its internal planning context.
- Exactly one installation is designated `founder-portfolio-authority` for an environment. Other Founder Hub installations mirror or adopt from it; they do not co-write its portfolio and there is no database leader election.
...
- A future hosted Founder Hub directory/relay may improve reachability. Local operation and direct links must not depend on it.
...
- Founder Hub receives governed code results through Hive Mind and, separately, higher-quality curated demand when a reseller chooses to share it.
...
The Founder Hub relationship supports three independently authorized channels. Enabling one never enables another.

| Channel | What Founder Hub receives | What remains local |
| --- | --- | --- |
...
The contribution result may carry an opaque source receipt so the originating installation can reconcile acceptance or release status. That receipt is not a dereferenceable backlog identifier. If the contributor also wants Founder Hub to understand broader demand or affected-customer reach, it publishes a separate demand envelope under its demand projection contract.
...
## 8. Founder Hub master shared portfolio

Founder Hub presents a network-demand view above its local Delivery Flow:
...
3. **Shared portfolio:** clusters accepted as founder-managed product demand, optionally linked to one canonical Founder Hub `BacklogItem`.
...
Founder Hub can delegate curation to named resellers for their channel without delegating final founder-portfolio disposition.

This view is populated only by Founder Hub-local demand and explicit demand-channel projections. Hive Mind contribution intake appears in the code-governance/contribution surface and does not create or reveal a remote backlog item. Founder Hub may create its own local follow-up item after reviewing a contribution, but that is a new Founder Hub-owned record linked to the contribution receipt—not a mirror of the contributor's backlog.
...
- Preserve pseudonymity end to end. A reseller cannot deanonymize a source for Founder Hub unless the source explicitly selected named attribution.
...
| One centralized Founder Hub SaaS backlog | Rejected as the only path. Simple global visibility but makes customer operation and channel trust depend on a central service and creates unnecessary custody. |
...
| **Selective hybrid: sovereign local truth + governed links + Founder Hub shared registry** | Selected. Kernel scoring returned high confidence (composite `7.173`, margin `1.254`, audit `DI-B5CEB8CFBBED`). It best balances sovereignty, reuse, privacy, channel value, and founder visibility. |
...
- Establish Founder Hub-owned reseller enrollment, agreement/entitlement, standing, and contribution-recognition views.
...
### Slice 4 — Founder Hub business, shared portfolio, and Hive boundary
...
- Add optional Founder Hub/reseller store-and-forward relay.
...
| V-07 | Reseller forwarding | Founder Hub receives original provenance plus reseller attestation; pseudonymous origin remains pseudonymous. |
...
| V-11 | Founder Hub outage | Customer and reseller local backlogs continue; queued demand later reconciles. |
...
| V-13 | Route loop | A customer ↔ reseller ↔ Founder Hub plus direct customer ↔ Founder Hub topology delivers one logical event and rejects cyclic forwarding. |
...
| Founder portfolio adapter | Federation mirror ↔ Founder Hub portfolio | Cluster, reach, disposition, canonical shared-item mapping |
| Hive contribution adapter | Contributor result ↔ Founder Hub Hive Mind | Result artifacts, verification, DCO, provenance, review/disposition; no source backlog |
| Forge adapter | Accepted Founder Hub result/item ↔ GitHub | Delivery mirror and contribution state, never federation or backlog authority |
...
- Founder Hub clustering and shared disposition allocate to the founder portfolio application layer, not the transport.
...
- Founder Hub: cluster membership, founder disposition, shared-portfolio item, and public delivery mapping.
- Founder Hub commercial authority: reseller relationship, agreement/entitlement, channel governance, and contribution recognition.
