...
| **For Employees** | AI Workforce + tax-remittance registry rows; integrations | productivity integrations (M365, Slack), internal tools the workforce consumes, AI coworker services/offers, finance/tax work lanes | AI Workforce (`used`), Tax Remittance and Paying Taxes (`used`), Microsoft 365 (`available`), Slack (`potential`) — workforce identities remain Facet B / 2026-06-07, while AI coworker products and capability needs are structurally attributable here |
...
- **P0 — Coverage & source schema audit + decision (keystone enabler).** Confirm where `coverageStatus`/`sourceKind` live (column vs JSON); confirm `InventoryEntity` can carry non-network sources without breaking source-scoped staleness; write the projector contract. *(small)* — **landed**: the projector contract + closed enums shipped with P1; the typed columns now land too — `DigitalProduct.coverageStatus` + `sourceKind` (nullable, additive migration `20260622030000_add_portfolio_coverage_columns`), populated by the writer alongside the `observationConfig` markers current readers use (reader migration to the columns is a trivial follow-up). The enums' source of truth stays in `portfolio-sources/types.ts`.
...
- **P4 — SBOM component projector.** Project `BomComponent` as the component layer of `dpf-meta`/`dpf-platform-standard` with supplier/license/PURL + paid flag; compose with EP-ASSURANCE-LEDGER. *(medium)* — **landed**: `project-sbom.ts` = curated `licensed-dependencies-manifest.ts` (always-present paid deps: Docker Business, Neo4j Enterprise — the npm SBOM is overwhelmingly free OSS and the paid third parties live in the procurement layer) **+** a `BomComponent` projector for the notable bill-of-materials (frameworks/apps/containers/models, OSS-library long-tail filtered; paid heuristic from license), dormant until an assurance/SBOM run populates `BomComponent`. Both → Foundational, source `sbom`. Wired into the seed.
...
- **P6 — Coverage surface UX.** Coverage filter/legend + provenance chip + "Enable" deep-link on `potential` rows; UX-fit gated; no new route. *(medium)* — **landed**: `CoveragePanel.tsx` at the portfolio root lists every entry by coverage (used/sold/available/potential) with a report-kit `StatusBadge` (new `portfolioCoverage` intent domain in `statusColors.ts`), a provenance chip (source), and a governed "Enable" deep-link to `/platform/tools/integrations/<slug>` for `potential` integrations. Root-only (inner taxonomy nodes keep the active-subtree list); reads coverage/source from `observationConfig`; no new route. Live-portal UX verification pending (source-only worktree).
- **P7 — Coverage/source parity guard.** A test asserting the projected portfolio stays in sync with the live substrates (mirrors the EP-SCHEDULING-SURFACE catalog↔registry parity guard) — so a new integration/capability/dependency can't silently fall out of the portfolio. *(small)* — **landed**: `portfolio-projection-parity.test.ts` asserts every static manifest (platform capabilities, licensed deps, supported integrations, archetype supply) yields well-formed entries (valid slug/coverage/source, non-empty name, one entry per item) with globally-unique, distinctly-prefixed productIds — so a malformed or colliding source row fails CI.
