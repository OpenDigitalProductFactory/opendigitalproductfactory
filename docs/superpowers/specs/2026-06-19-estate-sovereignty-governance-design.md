...
- **Phase 1 — Org-level assessment.** *(in progress)* The **CADA-readiness assessment** has landed: `computeInstallCadaReadiness()` (`packages/db/src/cada-readiness.ts`) derives the install's posture from the live local-only inference setting + declared jurisdiction + DPF's structural facts, scored by the primitive; surfaced via the `assessCadaReadiness()` server action and the `check-cada-readiness.ts` operator script (functionally verified against the live install). A complementary **governance-evidence lens** (`summarizeControlCoverage`) reports the implementation status of the seeded CADA controls (verified live: 4/8 implemented, the 3 planned being the Level-4 build-out). **Region/archetype applicability gating** (`packages/db/src/regulation-applicability.ts`, `regulationApplies`) now precedes any assessment: CADA is region-specific, so an org with no EU operating/selling nexus is reported *out of scope* rather than scored (verified live: undeclared install → "not applicable"; declared EU operating nexus → Level 3). This **generalizes** — every region- or archetype-specific regulation declares a `RegulationApplicability` (`basis` + `jurisdictions` + optional `archetypes`) and reuses the same `PROFESSION_JURISDICTION_BASES` vocabulary as the corpus layer, so applicability is consistent across knowledge injection and governance assessment. *(Candidate kernel principle: "assess only applicable regulations; regulatory applicability is region- and archetype-scoped.")* Still open: `BusinessContext.targetAssuranceLevel` as a **persisted** field (deferred — a schema regen in a junctioned worktree writes into the root clone's Prisma client, which is currently on another session's branch; target is a runtime param until this lands via an isolated client or Build Studio), and signed SBOM generation (Level 2, `EP-ASSURANCE-LEDGER`). Verifies REQ-CADA-3/6.
