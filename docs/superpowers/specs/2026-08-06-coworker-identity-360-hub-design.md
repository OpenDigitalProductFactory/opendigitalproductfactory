...
- **Phase 2 — Share + the whole-coworker agent-card + Teams facet. DELIVERED (2026-08-11).** The **Share** affordance (`components/platform/coworker-identity/ShareIdentity.tsx`) leads with a human identity card + copy-link; the machine-facing endpoint sits behind a "for agents / developers" disclosure — `GET /api/a2a/coworkers/[agentId]` (`app/api/a2a/coworkers/[agentId]/route.ts`) returns the whole-coworker `application/agent-card+json` via `projectCoworkerIdentityAgentCard` (`lib/coworker-service-catalog/agent-card.ts`), aggregating the coworker's A2A-exposed offers and reusing the offer card's access model (internal-a2a requires a session; partner/external scope-gated). The **Teams & collaboration ("work rooms")** facet (`lib/coworker-identity/teams-projection.ts` + `TeamsFacetPanel.tsx`) surfaces the coworker's `ValueStreamTeamRole` + `AgentOwnership` memberships. So other AI resolves the same identity a human sees, and the founder's "what work rooms are they part of" is answered — guts never on the front face.
