...
| Branch protection / required checks (GitHub) | Machine-checkable local-CI evidence when install can attest | Blocking all external forks without a local install (need fork policy) |
...
Narrow `Local-CI-Override` allowlist. `pr:health` fails closed on free-text agent overrides.  
...
A required CI/status check that queries the install gate store by PR head SHA.  
**Pros:** merge-queue reality. **Cons:** multi-install / fork design needed.

### Option D — Composite (recommended)
1. **Allowlist overrides** (A).  
2. **Surface hooks** (B).  
3. **Merge-readiness** (C lite).  
4. **Metrics** by client provider.
...
2. **Free-text Local-CI-Override cannot green-wash a runtime PR** — closed enum only.  
3. **At least one peer surface (Grok) hard-refuses** `gh pr create` / push of runtime code without unexpired pregate evidence for HEAD.  
4. **Claude/Codex paths remain valid** — their sandbox-using flow continues to pass.  
5. **Escape hatches remain for true emergencies** — operator-only, audited, never the default agent path.  
6. **Regression tests** prove: runtime PR without evidence = NOT READY; docs-only exempt; allowlisted override works.  
7. **Learning routed** — surface speed without pregate is a defect signal, not a productivity win.
