...
2. **BI — Org-identity context bundle for external MCP callers.** Compose mission + archetype doctrine + locale/currency/jurisdiction + stance into an ambient block at MCP `initialize` (instructions + resource + `get_org_context` tool), mirroring `prompt-assembler`. Relates EP-AGENT-INSTRUCTION-PLANE, EP-ORG-LOCALE-CURRENCY. Size: large. **PARTIALLY DELIVERED (BI-HDLEMP-02):** `apps/web/lib/mcp/org-context-bundle.ts` composes the bundle (mission from `BusinessContext`, archetype doctrine from `archetype-business-context`, locale/currency from `OrgSettings` via `org-locale`, the 5 stance vectors) and renders it into the MCP `initialize` `instructions` — including the `decisionDomain="org-business"` directive that activates item 1's engine (fail-open to the base note). **Deferred:** the pull-path `get_org_context` MCP tool and the MCP resource (new-tool gate surface), and folding profession jurisdiction into the block. *The `initialize` change carries context, not authority: it does not alter MCP tool authorization or the AI-agent meta-model — those remain as documented in `docs/architecture/mcp-tool-authorization-runbook.md` and `docs/architecture/ai-agent-meta-model.md`.* **Corrected (BI-HDLEMP-07):** live verification on the running install found that `principle_decide` ignores a `decisionDomain` hint (that param was reverted in item 1), so an external agent following the original directive still routed to WWMD. The directive now routes **business** decisions to `evaluate_org_business_decision` — the dedicated gate that reaches the org WWWD profile by construction (verified live: `orgProfileSelected=true`) — and **platform** decisions to `principle_decide`. The caller-context `decisionDomain` axis (item 1) remains available for a future `principle_decide` consumer but is not relied on by the directive.
3. **BI — Employee-scoped MCP credential issuance + on-behalf-of subject binding (headless-employee session).** Governed issuance binding a PAT to a specific `EmployeeProfile`'s `User` + org `agentId`; guarantee `User` backing; reconcile with the anti-impersonation Pseudo-User Contract (delegation + dual-principal audit). Extends EP-AGENT-AUTH-VAULT BI-6A01F90E / BI-717FD971 / BI-F52C13C2 (does not restate them). Size: large. **PARTIALLY DELIVERED (BI-HDLEMP-03):** `apps/web/lib/auth/employee-scoped-token.ts` — `resolveEmployeeTokenSubject` maps an `employeeId` to its backing `User` and **fails loudly when there is none** (`EmployeeProfile.userId` is nullable), and `issueEmployeeScopedMcpToken` composes that with the trusted `issueMcpApiToken` primitive under the **delegation contract**: subject = employee's User (accountable, carries role caps), actor = org `agentId` (REQUIRED — refuses to mint a bare act-as-human token). Authority flows through the existing `(role caps) ∩ (agent grants) ∩ (token scope)` pipeline unchanged, so this is a binding+validation layer, not new credential crypto. **Deferred:** the operator-facing issuance surface (portal/admin action + CLI path in `issue-mcp-token.ts`), guaranteeing/provisioning `User` backing at issue time, RFC 8693 token-exchange for short-lived delegated sessions, and org tenancy (`organizationId` on `McpApiToken`).
4. **BI — Close the external-surface authz holes.** Clearance-filter `tools/list`; require headless-employee tokens to be agent-bound; unify list/call authority so the list never advertises un-callable tools; fix `loadUserContext` multi-group role resolution. Relates EP-COMPANY-AUTHZ-FGA. Size: medium. **DELIVERED (BI-HDLEMP-04):** `apps/web/lib/mcp/listing-authority.ts` composes the two **static** coworker-authority axes a faithful `tools/list` must honour and the MCP route (`apps/web/app/api/mcp/v1/route.ts`) applies them to both `tools/list` and the `load_tools` candidate set: (a) **agent-grant parity** — a tool the acting agent's grants don't cover is dropped, using the same `isToolAllowedByGrants` predicate `tools/call` uses (closes the list/call skew); (b) **clearance filter** — when the acting human's `Principal.sensitivityClearance` doesn't cover the acting agent's data `sensitivity` (or the agent can't be resolved to an active identity), the **whole** agent-bound governed surface is dropped (only `load_tools` remains), mirroring the `sensitivity-clearance-denied` call-path deny. Clearance stays **distinct** from role (spec §7): role → `User.groups→PlatformRole`; clearance → `Principal.sensitivityClearance`; the shared `coerceDataSensitivity` (in `@dpf/db/principal-sensitivity`, now used by both the list filter and the runtime authority resolver so they can't drift) fails closed to `restricted` for any unknown label. `loadUserContext` now resolves the **full** multi-group role via the canonical `resolveWorkforcePlatformRole` (first group with a non-null role) — matching the web session — instead of the null-unsafe `groups[0]` (`take: 1`). Per-invocation axes (route-scope, subject-scope, approval/HITL) are intentionally **not** list-filtered (an approval-required tool is still listable). "Require headless-employee tokens agent-bound" is enforced at **issuance** (BI-HDLEMP-03: `issueEmployeeScopedMcpToken` refuses without an `agentId`); a non-agent-bound operator PAT is unaffected and retains the direct-human capability + token-scope contract. Runbook updated: `docs/architecture/mcp-tool-authorization-runbook.md`.
5. **BI — Query-time row-scoping on list/search MCP tools.** Bound `query_employees` and peer list tools to the acting human's manager/department visibility so the agent sees exactly what the person would. Relates EP-COMPANY-AUTHZ-FGA, `manager-scope.ts`. Size: medium. **DELIVERED (BI-HDLEMP-05):** `query_employees` (`apps/web/lib/mcp/packs/workforce-pack.ts`) now applies a **query-time** `id IN (visible set)` filter. The visible set is `employeeScopeVisibleIds` (new, in `apps/web/lib/govern/manager-scope.ts`) — the row-level **dual** of the existing `canAccessEmployeeScope`: superuser → unrestricted; everyone else → self ∪ direct-reports ∪ indirect-reports. This closes the seam where `deriveCoworkerAuthoritySubject` resolved a *list* call to the `platform` subject and so bypassed the `subject-scope-denied` gate that already scopes id-addressed employee tools — the list now returns **exactly** the rows the caller could open individually (proven by a unit test asserting `id ∈ visibleProfileIds ⇔ canAccessEmployeeScope(ctx, id)`). Scope is resolved from the acting human's `userId` via the same `resolveManagerScope` the effective-auth loader uses, so list-scope and record-scope can't drift. The `platform`/reference list tools (`list_departments`, `list_positions`) are not employee rows and stay unscoped; other-domain list tools (CRM etc.) use account-scope, tracked under EP-COMPANY-AUTHZ-FGA. Department-level visibility (beyond the manager chain) is a future extension — the current `canAccessEmployeeScope` is manager-chain only, and matching it exactly is the correctness guarantee here.
