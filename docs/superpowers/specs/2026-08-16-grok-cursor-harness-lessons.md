...
- **The Grok-4.5 / Cursor data flywheel** — Cursor turned proprietary interaction traces into a co-trained frontier model. DPF (this operator install) sits on comparable cross-install governed-decision data; instrument it as a curatable corpus, but the privacy model must lead (Cursor's "paths encrypted, code never stored plaintext, decrypt client-side" as the hard constraint before any cross-install learning ships).
