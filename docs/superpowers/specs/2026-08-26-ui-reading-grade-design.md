...
Recorded here rather than as a separate plan document. The plan-coverage gate requires
every `docs/superpowers/plans/` file to carry a receipt from `record_plan_backlog_coverage`,
and that tool refuses this caller with `gate-not-authorized`. Rather than ship a plan file
that can never pass its own guard, the phases live here.
