...
- **P3 convergence — slices 1–2: Finance + Admin (BI-89F03D6E)** — `apps/web/lib/ea/domain-nav-sources.ts`: registers the per-domain nav components as additional navigation sources via **read-time unification** (not a second copy of the data). A generic `familyNavSource` flattens the pure-data family navs `FINANCE_FAMILIES` + `ADMIN_FAMILIES` (the two biggest orphan clusters, 39 + 14) to `NavSourceEntry` rows with teleport-aware `targetDomain`; `getAllNavEntries()` merges canonical + per-domain, deduped by path. Both the reconcile projection and the P7 gate now read `getAllNavEntries()`, so Finance's + Admin's section routes (invoices, bills, banking, settings, branding, diagnostics, …) join the navigation surface, gain `navigates-to` edges, and leave the orphan set. Subsequent slices ingest the `.tsx` TABS navs (compliance/ea/ops/customer-marketing/employee/storefront) as their arrays are extracted to pure data modules (composes P5); the eventual end state has each per-domain `TabNav` render from the one model (the Customer nav is the existing exemplar).
