...
- **CORRECTION — P1/P2 console-unify reverted (founder feedback, live, 2026-06-22).** On the deployed build the founder reported "jumping around the sub menus to other main menu sections": the unified `ConsoleTabNav` put `Administration → /admin` and `Runtime & Releases → /ops/self-upgrade` tabs into the platform secondary nav (and the platform families into the admin row), so a secondary-nav click crossed into a *different* left-rail main section. That re-created the very "Core Admin teleport" the keystone removed — just renamed. **Resolution: a secondary nav navigates only within its own main section; crossing sections is the left rail's job (+ the global breadcrumb for the way back).** Reverted `console-nav.ts` / `ConsoleTabNav` / the `PlatformTabNav`+`AdminTabNav` re-exports — Platform shows only platform families, Admin shows only admin families, neither links out. The keystone (no teleport + breadcrumb) was the right-sized fix; P1's full console-merge over-engineered it. Also fixed: the `/ops` page rendered `OpsTabNav` *below* the large Escalations panel — moved it to the top. (An alternative that would keep one console is to consolidate the **left rail** into a single "Operator Console" area owning platform+admin+runtime, so the unified secondary nav no longer crosses rail sections — deferred pending founder direction.)
