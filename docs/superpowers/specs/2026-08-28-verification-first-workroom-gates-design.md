...
**Installation-qualified reconciliation, 2026-09-06:** the earlier
`not_found` observation above must not be interpreted as global absence.
`get_backlog_item` on the operator development install returns the non-vacuous verification work and the deep-verification integration work as
...
Phase 4 still requires demonstrated completion of the non-vacuous verification work, not merely a row.
...
Research: the linked research, [source review](../research/2026-09-06-astra-business-verification-review.md).
...
The non-vacuous verification work owns trustworthy result production; the deep-verification integration work owns later gate binding.
