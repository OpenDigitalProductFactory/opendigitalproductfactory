...
| 11 | BI-348766E5 | **`storefront-demand` source** — an unhandled customer record (a `StorefrontInquiry` awaiting reply, or a `StorefrontBooking` that is `pending` confirmation or `overlapQuarantinedAt`-double-booked) is projected as a `products-and-services-sold` item carrying guest/ref, table/service, date/time, status, and one exact next action. A `customer-waiting` residue with a **hard floor** into `needs-you-now` at every proactivity level, so Workspace becomes the single owner-readable truth for restaurant demand — the cockpit can never read "Nothing needs you right now" (nor a twin "Reservations 0 · Clear") while a customer record is outstanding. Source rows stay canonical in the storefront tables; nothing is persisted. | S | Closes the split-truth gap where demand lived only in the storefront-local inbox and Workspace/Needs-You stayed silent. Restaurant cards sort ahead of generic fallback decisions; repeated action verbs are accessibly disambiguated (Label-in-Name safe). Context: BI-075F731F. |
