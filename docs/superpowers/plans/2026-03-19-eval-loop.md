...
const CONTEXT_RETENTION_TESTS: GoldenTest[] = [
  {
    ...
    prompt: "Earlier in our conversation, I mentioned that the project codename is 'Phoenix'. What was the project codename?",
    scoring: "retrieval",
  ...
  },
  {
    ...
    version: 1,
    dimension: "contextRetention",
    prompt: "I told you my team has 7 developers and we use Rust. How many developers are on my team and what language do we use?",
    scoring: "retrieval",
...
describe("scorePartial", () => {
  ...
  it("returns 5 for partial match", () => {
    expect(scorePartial(
      "The money adds up differently than you think.",
      "There is no missing dollar",
    )).toBe(5);
  });
  it("returns 0 for completely wrong answer", () => {
    expect(scorePartial("The missing dollar is in the bellboy's pocket.", "There is no missing dollar")).toBe(0);
  });
});
...
function scoreResponse(
  ...
  switch (test.scoring) {
    ...
    case "orchestrator":
      // Orchestrator scoring requires a separate grading call — return 5 (neutral) as placeholder.
      // Full orchestrator grading is handled by the eval runner's main loop.
      return 5;
    default:
      return 0;
  }
...
export async function updateEndpointDimensionScores(
  ...
  // Check golden test recency — skip if evaluated within 24 hours
  const provider = await prisma.modelProvider.findUnique({
    where: { providerId: endpointId },
  ...
  // Update EndpointTaskPerformance
  if (perf) {
    await prisma.endpointTaskPerformance.update({
      where: { id: perf.id },
      data: { dimensionScores: currentDimScores },
    });
  }
