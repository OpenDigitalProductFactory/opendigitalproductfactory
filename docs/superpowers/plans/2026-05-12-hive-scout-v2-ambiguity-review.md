...
- [x] **Cache TTL sanity check.** Confirm the 30-day default against the upstream catalog's actual update cadence (per spec §8.2 open decision #2). *(Measured 2026-05-15 against `repos/ashishpatel26/500-AI-Agents-Projects/commits`: 47 commits across 16 distinct burst-days from 2024-12-20 to 2026-01-13. **Inter-burst gaps in days: median 21, mean 25.9, max 94, min 1.** 60% of bursts happen within 30 days of the previous one; the most recent burst was 122 days before the measurement.* **Decision: keep the 30-day default.** *Reasoning: upstream README cadence is the wrong proxy for this cache TTL. Our cache is per `sha256(sourceUrl)` — a previously-reviewed entry's classification stays valid as long as the entry's content is stable, which is decoupled from how often the README is touched (most upstream commits add new entries that have no cache row yet, or fix typos that don't change classification). The right invalidation trigger is **DPF's own skill/coworker registry churn**, not upstream. 30 days is conservative against that. Operators who want re-validation faster (e.g., during a use-it-or-lose-it window per spec §6) can lower `hive-scout.review.cacheTtlDays` via `PlatformConfig` without a deploy.)*
