...
**Local cold-start checkpoint (2026-07-20):** `BI-EDAAD429` now carries a hard `local_only` residency boundary from the provider-compliance collaboration runtime through the canonical agentic route options, accepts only bounded non-approval local output, and falls back to a deterministic cited advisory when the local model is absent, malformed, or attempts an unsupported approval. Runtime/build evidence remains required before the BI is closed.
