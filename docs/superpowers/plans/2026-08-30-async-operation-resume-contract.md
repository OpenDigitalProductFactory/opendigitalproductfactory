...
- Make the exact constrained durable route posture-inert: do not resolve or inject saved coworker, organization, or platform Golden Triangle posture into its route context or execution plan. Add a non-Balanced saved-posture fixture proving the pinned recipe remains exact while ordinary unconstrained routing retains its posture behavior.
