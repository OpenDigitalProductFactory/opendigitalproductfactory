...
**Live functional verification (2026-08-07, against the live org WWWD gate).** Ran the real `buildLeaveScoredOptions` vectors through `evaluate_org_business_decision` for a clean case and a tight-coverage case. Both returned `recommend/approve`, confidence 0.825, **`orgProfileSelected: true`** (grounded in the org's own stance — the install is configured as a food-hospitality/restaurant org), with real ledger `interactionId`s (`DI-CEAF442A565D`, `DI-16781F232B81`). **Finding:** the tight-coverage case *also* recommended approve — because (per `option-recommendation.ts`) the approve-vs-deny pick is a **commandment-argmax** and confidence is **profile-level**, so the leave-specific `business_disruption` axis barely influences the gate's option pick. **Consequence:** coverage-tightness sensitivity must live in the deterministic guards, not the gate. **Acted on it (D2.1 refinement, tested):** added a configurable `minCoverageCushion` to `evaluateLeaveGuards` — default 0 (only a true breach escalates, unchanged), but a coverage-sensitive org (food-hospitality, where running to the bone risks the guest experience) sets it to 1+, so approving down to zero cushion escalates to a human by construction. The I/O wrapper will pass the org's configured cushion.
