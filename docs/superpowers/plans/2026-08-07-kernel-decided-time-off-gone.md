...
The Greenhouse effort (`EP-ECOSYSTEM-ABSORPTION-ARCH`: `BI-E5561DC9` bridge→absorb→replace, `BI-9CC44DC7` recruiting pipeline surface, `BI-02F1F944` hire→`EmployeeProfile` landing) is **recruiting/ATS**, so there is **no functional overlap** with leave decisioning. The overlap is shared *infrastructure* — coordinate on these, do not fork:

| Shared substrate | Greenhouse/recruiting use | This plan (D2) | Rule |
| --- | --- | --- | --- |
| `pack-registry.ts` + `agent-grants.ts` | `recruiting-pipeline-pack` already landed | adds `leave-decision-pack` + mirrored grant | Union-merge; **update `TOOL_TO_GRANTS` or the drift test fails** |
| `AgentActionProposal` / `CoworkerActionEnvelope` | proposal substrate | new `actionType:"leave.decide"` value | New enum value, **not** a new table |
| `EmployeeProfile` (worker record) | `BI-02F1F944` lands hires into it | leave decisions attach to it | Read-only from D2; no schema contention |
| Coworker-surface convention | recruiting ships an **MCP tool over a shared read-model, defers the UI page** to dodge the route/ux-fit gauntlet | D2 same: MCP/lens surface + render on existing `LeavePanel`; **no new ratified route** | Avoids `page-purpose` + `ux-fit` sweep |
| `lib/actions/leave.ts` hardening | — | D1 wraps it in governance | **Overlap-check before editing** — the governance-log gap is well known; confirm no peer session is mid-edit (`gh pr list` can't see in-flight peers — grep the file's recent history) |

**The strongest reuse precedent is the AI *staffing* coworker, not recruiting** (recruiting is a read-only lens): reuse `AgentActionProposal` + `prepareStaffingProposal` + `authority.ts` + the kernel-consult-ledger. This keeps both absorptions presenting one consistent "AI coworker proposes over a record, records a sealed rationale, escalates by exception" pattern.
...
**Touched files.** New `lib/workforce/leave/decide-proposal.ts` (pure payload, mirrors `prepareStaffingProposal`); new `lib/mcp/packs/leave-decision-pack.ts` + `pack-registry.ts` entry + `agent-grants.ts` mirror; extend `lib/workforce/staffing/coworker/authority.ts` for the *propose* action only; migration adding `LeaveRequest.decisionInteractionId String?` (+ optional `rationale Json?`); `components/employee/LeavePanel.tsx` renders the recommendation + rationale; coworker definition via `establish_coworker`.
