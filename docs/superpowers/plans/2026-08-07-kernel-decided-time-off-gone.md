...
- **D2.2 — kernel option/feature model + orchestration** (next): map the leave inputs to `DecisionOption` features on real `PRINCIPLE_DIMENSIONS` axes (cost-direction aware — see `dimension-catalog.ts`), call `principleDecide`, `mapConsultOutcome` → `LeaveKernelOutcome`, feed `resolveLeaveDecision`. The feature-axis choice is a design decision worth a `dpf-decision-via-kernel` pass before finalizing.
