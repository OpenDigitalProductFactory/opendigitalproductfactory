...
**Integration checkpoint (2026-07-18):** Production startup now invokes the concrete read-only receipt adapter as an awaited boot barrier. Durable envelopes are signature-checked before replay; only live envelopes may relaunch; expired/missing receipts invoke a signed reverse projection and a verified compensation receipt; nullable legacy envelope identity and failed compensation become `recovery_required`. Admission consults DB-global active/recovery rows rather than module state alone. Contributor installers create an owner-only host signing key on Windows and macOS/Linux, the portal receives only a read-only state mount plus host path metadata, and the long-lived Compose promoter no longer mounts state read-write. Steps 7-9 remain unchecked: an authenticated capability mutation surface, complete shared producer advisory-lock coverage, canonical install-state schema reuse, explicit rotation orchestration, and canonical-runtime recovery/rollback evidence are still required.
