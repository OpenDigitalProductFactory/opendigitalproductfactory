...
- **Phase R6 — "what's shared" visibility (B5 pillar 4 / spec §8.1 customer vendor view).** Adds a read-only **Shared scope** column to `/platform/federation-links`: per link, the minimum-necessary projection that actually crosses to the peer (slices + retention), computed with R5's `resolveIncidentProjectionSpec` over the link's `proposedProjection` (else the safe default). This is the trust surface for the regulated-customer case (a law/doctor firm SEES what is exposed to the MSP, not just trusts it is minimal) — it reads the SAME spec the egress gate enforces, so the display can't drift from reality. No new model/route/migration; one column + a caption. The customer-editable *narrow-the-scope* control (materialize a `ProjectionContract` row + an editor) is the next increment. apps/web typecheck clean; dialog-guard clean.
