...
1. **BI-PORTPRIO-1 — backlog→portfolio attribution.** `resolveBacklogPortfolio()` (digitalProduct→taxonomyNode→epic precedence) + denormalized `BacklogItem.portfolioId` (additive) + per-portfolio grouping read. *Enabler.* — **Landed**: `packages/db/src/backlog-portfolio.ts` (`resolveBacklogPortfolio` + `attributeBacklogPortfolio` on-create primitive + `backfillBacklogPortfolios` + `getBacklogByPortfolio` grouping with effort breakdown) + `BacklogItem.portfolioId` column/index (migration `20260623040000`); wired into the ops `createBacklogItem` action (populate-on-create) + a seed backfill step. Other create paths (MCP/findings/triage) are covered by the backfill until wired.
...
3. **BI-PORTPRIO-3 — budgets as real funding envelopes.** *(2026-09-24: carried forward as quarterly points budgets with reservations — [design](../specs/2026-09-24-portfolio-budget-and-investment-wip-design.md), EP-PORTFOLIO-BUDGET-WIP.)* Operator-set, persisted `Portfolio.budgetKUsd` (provenance flips demo→live) + budget-vs-allocated surfaced on the portfolio root.
...
5. **BI-PORTPRIO-5 — cross-portfolio cascade edges.** *Landed*: `crossPortfolioDependencyEdges()` in `product-dependency.ts` — sold (`dpf-platform-standard`) → Manufacturing & Delivery pipeline (Build Studio / GitHub / self-upgrade) + every supported integration → the foundational MCP plane, emitted by the seed `productDependencyGraph` step. Extends the cascade beyond Foundational to **Manufacturing & Delivery** (and the integration tools downward to Foundational); the **sold→workforce** reach into **For Employees** is gated on Facet B (BI-554E1A14 — the workforce is not a portfolio entry yet).
