...
| File | Responsibility |
|---|---|
| `packages/db/prisma/migrations/20260408110000_add_agent_convention/migration.sql` | Migration for AgentConvention model |
...
| `apps/web/lib/integrate/specialist-prompts.ts` | Make `buildSpecialistPrompt` async, inject conventions |
...
- Create: `packages/db/prisma/migrations/20260408110000_add_agent_convention/migration.sql`

- [ ] **Step 1: Add the AgentConvention model to schema.prisma**

Open `packages/db/prisma/schema.prisma` and add after the `AgentPerformance` model (around line 1414):
...
**Files:**
- Modify: `apps/web/lib/integrate/specialist-prompts.ts` (lines 291-310)
- Modify: `apps/web/lib/integrate/build-orchestrator.ts` (lines 359-364)

- [ ] **Step 1: Make `buildSpecialistPrompt` async and inject conventions**

In `apps/web/lib/integrate/specialist-prompts.ts`, change the function signature and add the conventions query.

Replace the existing `buildSpecialistPrompt` function (lines 291-310):

```typescript
/**
 * Build the full system prompt for a specialist.
 * Composes: role prompt + learned conventions + task description + build context + prior results.
 * Async because it queries AgentConvention from the database.
 */
...
- [ ] **Step 2: Update the call site in build-orchestrator.ts**

In `apps/web/lib/integrate/build-orchestrator.ts`, the `dispatchSpecialist` function calls `buildSpecialistPrompt` at line 359. It's already inside an async function, so just add `await`:
...
The return type change from `string` to `Promise<string>` may surface other call sites. Fix any that appear.
...
In `apps/web/lib/tak/prompt-assembler.ts`, add an optional field to `PromptInput`:
...
export type PromptInput = {
  ...
  agentConventions?: string | null;  // ← ADD THIS (optional to avoid breaking other callers)
};
```

- [ ] **Step 2: Inject the conventions block into prompt assembly**

In the `assembleSystemPrompt` function body, add the conventions block after the domain context block and before route data. Find where `routeData` is appended and add before it:

```typescript
  // Learned conventions (maturity-gated, from AgentConvention table)
  if (input.agentConventions) {
    blocks.push(input.agentConventions);
  }
```

- [ ] **Step 3: Pass conventions from agent-coworker.ts**

In `apps/web/lib/actions/agent-coworker.ts`, in the `sendMessage` function, fetch conventions before calling `assembleSystemPrompt`. Find where `assembleSystemPrompt` is called and add the fetch:
...
```typescript
  ...
  const systemPrompt = assembleSystemPrompt({
    hrRole: user.platformRole,
    grantedCapabilities: granted,
    deniedCapabilities: denied,
    mode: coworkerMode ?? "advise",
    sensitivity,
    domainContext: finalDomainContext,
    domainTools,
    routeData,
    attachmentContext,
    agentConventions: agentConventions || null,  // ← ADD THIS
  });
```
...
In `apps/web/lib/integrate/build-orchestrator.ts`, find the block that saves `verificationOut` (around lines 617-634). After the existing `try/catch` block that saves verification evidence, add a new fire-and-forget block:
...
Insert this after the block that saves `taskResults` (after line ~683) and before the auto-advance block.
...
In `apps/web/lib/tak/orchestrator-evaluator.ts`, find where `updateAgentPerformance` is called (around line 154-158). After that block, add the convention classification trigger:
...
In `packages/db/src/seed.ts`, add the seed data loading near the top of the file (using the existing `readFileSync` + `JSON.parse` pattern, not JSON import assertions):

```typescript
// Near top of file, with other data reads:
...
const agentConventionsData = JSON.parse(
  readFileSync(join(DATA_DIR, "agent_conventions.json"), "utf-8"),
) as { conventions: ConventionSeed[] };
```

Add the seed function (uses proper `upsert` since `@@unique([agentId, category])` exists):

```typescript
async function seedAgentConventions(): Promise<void> {
  const conventions = agentConventionsData.conventions;
...
Add the call in the `main()` function after existing seed calls:
...
Ensure no existing tests were broken by the async change to `buildSpecialistPrompt`.
...
| 5 | Inject into specialist prompts | `specialist-prompts.ts`, `build-orchestrator.ts` |
