...
provision fixtures concurrently with the canonical CI build, then rendezvous
with the exact named artifact in the current run. Manual baseline calibration
remains independently dispatchable and builds locally.
...
defect. The first implementation experiment also proved that the visible poll
is mostly productive overlap with `Production Build`, not removable idle time:
PR #3729's direct `needs: build` topology delayed fixture setup and reached the
route crawl about 52 seconds later than PR #3726. The orchestration defect is
the cross-workflow event/SHA/source-run discovery, while the useful fixture,
browser, and build overlap must remain.
...
GitHub's supported contract keeps the artifact in one workflow run. Cross-run
download requires the token and source-run identifier that forced the current
locator. The corrected topology uses the known current run id and exact artifact
name, polls only that inventory while the producer is active, and keeps setup
parallel rather than applying a job-level dependency. Merge-queue required
checks must still trigger on `merge_group`.
...
CI workflow
  changes -------------------------+
        |                           |
        v                           v
  Production Build          UX fixture + browser setup
        |                           |
        | exact-tree archive       | current-run rendezvous
        +---------------------------+
                                    |
                                    v
...
- add failing source-level conformance assertions that PR and merge-group UX:
  ...
  - start from the change classifier without serializing setup behind `build`;
  - download the current run's exact artifact without workflow, event/SHA, or
    source-run discovery;
  - stop the current-run rendezvous when the known producer is terminal;
...
- start UX setup from `changes` in parallel with `build`;
- rendezvous only with the exact current-run artifact after setup;
...
- measured route-crawl start is no slower than the prior overlapping topology;
  remaining wait is classified as active producer build time, while
  transfer/materialization, portal startup, and route crawl remain separately
  measured.
