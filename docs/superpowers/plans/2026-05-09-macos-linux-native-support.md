...
- `docker-compose.macos.yml` **(new)** — Docker Desktop + Model Runner
  defaults; optionally points at host `node_exporter` on
  `host.docker.internal:9100`.
- `docker-compose.linux.yml` **(new)** — adds `ollama` service; opts in to
  the `linux-monitoring` profile.
- `monitoring/prometheus/prometheus.yml.tmpl` **(new)** — generated by
  installer, replaces hardcoded `windows-host` job at scrape target.
