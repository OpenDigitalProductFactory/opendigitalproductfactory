...
- **P0.1 — Architect decisions.** Resolve: **base access profile** — spec §5.6 now **recommends Option B** (add `HR-600` "Workforce Member" or an equivalent first-class workforce access profile) over reusing an `HR-*` platform-management role, because the six existing roles all carry platform-facing meaning/capabilities; the architect must pick the exact shape since it touches the strongly-typed role enum (AGENTS.md §3). Also resolve: coworker-roster storage (embed vs join); completion tracking (`OnboardingTask` rows preferred, derived `platformIntroCompletedAt` only if perf requires); occupation override policy; the "show all my access" reveal permission grant. Record via `dpf-decision-via-kernel` where a genuine trade-off exists.
