...
- **P2.1 — Occupation axis in the resolver. SHIPPED.** `resolveWorkspaceHomeContribution` now takes an `occupationKey` and resolves over the five-tier order in spec §5.3 (archetype×occupation → category×occupation → archetype → category → platform), most-specific-wins; `WorkspaceHomeContribution.occupationKeys?` + two new match kinds (`archetype-occupation`/`category-occupation`) added additively. Archetype tiers are guarded (`!isOccupationScoped`) so an occupation home never leaks to every worker of the archetype; absent `occupationKey` is byte-for-byte the prior behavior. Reuses the existing manifest/primitive/slot covenant unchanged. Note: the registry is **no longer dormant** (6 archetype contributions live via `DEFAULT_WORKSPACE_HOME_CONTRIBUTIONS`), so occupation homes slot alongside real archetype homes. **Registering the first occupation home contributions** (clinic hygienist, front-desk; field-service tech) moves to P2.2's slice with the nav gate + page threading so they land together as a visible surface.
