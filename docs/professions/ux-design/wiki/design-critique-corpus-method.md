...
**The screenshot itself lives in the shared MediaAsset store, served privately.** The founder-facing form uploads the image through the existing `POST /api/media` path, which content-addresses and deduplicates it (so a before/after pair of identical bytes collapses to one blob) and returns an asset id. `captureCritiqueEntry` then anchors that asset to the entry's page with a `MediaAttachment` (owner type `WikiPage`, role `critique-screenshot`) — the anchor is both what keeps the image from ever being reclaimed as an unreferenced asset and the scope its serve route checks. `screenshotRef` is set to `/api/critique-media/<assetId>`, a **session-gated, org-scoped** route — deliberately *not* the public `/api/media/:id`, because a critique captures internal product surfaces (`/admin`, live workspace data) that must never be servable by capability URL to an unauthenticated party. This storage/serve split was decided against the kernel (interaction `DI-D80467D6B45B`, option *authenticated-serve*), which chose it over reusing the public path and over building a separate blob store.
