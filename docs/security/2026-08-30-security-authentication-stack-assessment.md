...
| Customer/social authentication | Customer identities converge to Principal and inactive principals cannot authenticate. | Customer password and provider-subject social paths call the same population-aware Principal authority seam before session issuance; onboarding/linking/signup converge contact, Principal, aliases, and social identity transactionally; effective auth rechecks live Principal, contact, account, and scope state. | Implemented by BI-E22C3D75. Email alone never selects a social identity, alias conflict fails closed, and the populated-data invariant is exposed by `CustomerPrincipalAuthInvariant`. | Monitor the invariant aggregate after upgrade; repair any non-zero conflict count before re-enabling an affected identity. |
...
5. **Delivered—BI-E22C3D75:** customer/social sign-in now consults the Principal spine before session issuance. **Next—BI-DD3BBD02:** migrate social-provider secrets into the canonical credential store.
