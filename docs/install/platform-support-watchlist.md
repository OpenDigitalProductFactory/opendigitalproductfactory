...
| D13 | Login form looks inert on a valid password — post-login (or any Host-derived) redirect points at `http://0.0.0.0:3000`, which the browser cannot load, and no error is shown | all (the portal binds `0.0.0.0` in-container; `PUBLIC_URL`/`AUTH_URL` unset on a default local install) | ✅ BI-86165533: `apps/web/lib/reachable-url.ts` rewrites the unspecified bind-all host (`0.0.0.0` / `::`) to loopback; wired into `getPortalUrl` and a new Auth.js `redirect` callback (`apps/web/lib/govern/auth-redirect.ts`) so `trustHost`-derived redirects land on a reachable host. | Never build a browser-facing redirect straight from the raw `Host`/`x-forwarded-host` header without normalizing the bind-all wildcard; set `PUBLIC_URL` when a real external host exists. |
