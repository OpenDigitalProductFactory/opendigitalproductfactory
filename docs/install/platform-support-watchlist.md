...
| D11 | Nearby DPF instances do not appear even though both portals are healthy | Windows and macOS Docker Desktop; segmented enterprise LANs | Containers cannot bind the physical host's multicast interfaces, host firewalls may block UDP 5353, and mDNS is link-local by design. | ⚠️ Slice 1 allocates RFC 6762/6763 advertise/browse to the native Go Edge Node, not the portal container. Installer/service work must enable the host process and the narrow multicast firewall rule explicitly; cross-VLAN and Internet links use invitation or a future governed discovery proxy. | Never add mDNS to the base Compose topology or infer ownership/trust from a service record. Verify add/remove behavior on both Windows and macOS and inspect TXT data before closing V-01. |
