...
- **Conduit, not broker** — customers bring their own credentials (encrypted per-org); raw third-party payloads stay external. No DPF-as-third-country-broker exposure. ([native-cohesion-over-interfacing](../founder-kernel/wiki/principles/native-cohesion-over-interfacing.md))
- **One data model** on the customer's own Postgres — no data scattered across third-country SaaS. ([one-data-model](../founder-kernel/wiki/principles/one-data-model.md))
