...
- **Not yet / follow-on:** Phase 2 — surface provider-detect in onboarding + let a coworker drive the detect-and-guide flow conversationally (the fuller "AI-agent-assisted" realization). Per-install sending limits + abuse controls remain properties of whatever relay an operator runs (DPF operates none).
