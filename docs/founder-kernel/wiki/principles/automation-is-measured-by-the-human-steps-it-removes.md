...
**The tiers differ in how their cost accrues, not only in how much.** A human step and an AI token are **recurring** — paid again on every execution, scaling with usage. Code is **one-time build plus maintenance** — it does not scale with volume. So moving work down the ladder converts a variable cost into a fixed one, and for an operator funding the platform before it earns, that conversion is often the decisive argument rather than a secondary benefit: a recurring cost grows with adoption, which is exactly when a pre-revenue platform can least afford it. This is also why the ladder is not an argument for spending tokens freely on the way down — the spend is real, it is borne by whoever funds the platform, and an automation that never reaches code keeps charging for itself.

*Resolver note: the AI side of this ledger needs per-thread and per-run token and cost attribution to be real. That machinery shipped inert — the per-thread cost ledger (BI-CCF1ACBB) joined on a `threadId` column nothing populated, so it had never reported a number. The attribution fix landed with BI-CCF1ACBB's re-opened delivery; the measure becomes available on the AI tier only when that item's registered liveness probe confirms a non-zero ledger on a live install, not when the code merged. Dollar cost per run is still unpriced (`estimatedCostUsd` is unwritten), so the AI tier reads in tokens until pricing is joined.*
