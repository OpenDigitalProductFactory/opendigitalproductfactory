...
- **Platform function never depends on a client.** Every guarantee runs server-side, on every install, with no AI client present; a hook may accelerate one, never own it. Every agentic process owes the shared recovery contract, including executor replacement and credential rotation. **Commandment tier.** → [kernel principle](docs/founder-kernel/wiki/principles/platform-function-never-depends-on-a-client.md)
