...
- **An enforcement refusal is a stop, not a workaround.** Branch protection, the DCO bot, token grant-intersection and — where the host loads them — `PreToolUse` hooks refuse rather than warn. Surface the requirement and stop; never route around it via DB, filesystem or shell. ⟦situational: hooks are Claude-format — native on Claude Code/Grok, aliased on Codex, unproven elsewhere; on an unverified host honour the hook-enforced rules yourself — review at EP-ANTIGRAVITY-001's evidence gate⟧ → [PR](docs/founder-kernel/wiki/principles/all-changes-land-via-pr.md) · [DCO](docs/founder-kernel/wiki/principles/dco-sign-off-required.md) · [lease](docs/founder-kernel/wiki/principles/runtime-gates-via-shared-lease.md)
- **The canonical runtime is the only source of runtime truth.** A hand-built image or worktree-local server proves nothing about the live system. Runtime-bound verification, release validation and install advances route through the canonical runtime or the shared local-CI sandbox lease; the live install advances only via `/ops/self-upgrade`, and an image carries the identity of its bytes. → [worktree](docs/founder-kernel/wiki/principles/worktree-is-source-control-not-runtime.md) · [release QA](docs/professions/release-service-management/wiki/release-qa-plan.md) · [image identity](docs/founder-kernel/wiki/principles/image-identity-equals-bytes.md)
...
- **`principle_decide` auto-persists to `DecisionInteraction`.** Read `data.ledger` (`recorded`, `interactionId` DI-*). Operator audit is `/coworker-decisions/decisions` — not wiki `pageKind=decision` DEC pages and not free-form build notes. → skills [`dpf-decision-via-kernel`](packages/dpf-skill-pack/skills/dpf-decision-via-kernel/SKILL.md), [`dpf-record-decision-outcome`](packages/dpf-skill-pack/skills/dpf-record-decision-outcome/SKILL.md)
- **Pass a normalized `callingSurface` on every consult.** Use `grok-desktop` | `claude-desktop` | `codex-desktop` | `antigravity-desktop` | `build-studio` | `coworker` (optional `:<slug>`). Free-form labels fragment multi-surface adoption metrics (BI-D5ACBAE2).
