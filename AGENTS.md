...
- **An enforcement refusal is a stop unless the gate already exposes a checked-in, audit-recording override and the operator explicitly authorizes that override for this exact run.** Use only that supported path, record its reason and scope, and report the gate as **skipped/unrun — never passed**. No override may bypass branch protection, DCO, token grant-intersection, destructive-safety controls, or production-integrity controls; never route around a refusal through direct DB, filesystem or shell mutation. If no supported override exists, surface the requirement and stop. ⟦situational: hooks are Claude-format — native on Claude Code/Grok, aliased on Codex, unproven elsewhere; on an unverified host honour the hook-enforced rules yourself — review at EP-ANTIGRAVITY-001's evidence gate⟧ → [PR](docs/founder-kernel/wiki/principles/all-changes-land-via-pr.md) · [DCO](docs/founder-kernel/wiki/principles/dco-sign-off-required.md) · [lease](docs/founder-kernel/wiki/principles/runtime-gates-via-shared-lease.md)
