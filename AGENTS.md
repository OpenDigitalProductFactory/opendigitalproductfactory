...
- **Autonomy requires `signalQuality.autonomyEligible`, not mere recommendation.** Math is weighted-sum MCDA (WSM), not Saaty AHP eigenvectors. Gates: ≥3 feature axes/option, weight-sensitivity stable, high confidence, no commandment conflict (BI-1D23EC26). → [research](docs/superpowers/research/2026-08-10-decision-vector-science-and-corpus-adequacy.md)
- **Score ≥3 feature axes per option** on every multi-option platform consult so structured scoring can discriminate; thin feature maps force low confidence and block unattended proceed.
