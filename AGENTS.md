...
- **Work-scope / altitude decisions are platform-owned (WWMD) — reuse settled direction; consult open trade-offs, don't ask cold.** How much to take on in a pass is a platform decision: reuse applicable founder direction or a valid decision; route an unresolved trade-off through `principle_decide` **before** surfacing a menu. Act on a high-confidence recommendation and report the ledger; escalate only on low confidence or a commandment conflict. "Option 1/2/3, you pick" without a ledger is the anti-pattern. A `[governance-freshness]` warning means treat every decision as guard-off and route it manually.
...
- **Governance approves relevant evidence, not provenance or ceremony.** A gate serves a named decision or protection; reuse valid evidence and authorization, and remove redundant collection or approvals. A phase transition alone does not require fresh human consent. It never asks which client produced the evidence. → [kernel principle](docs/founder-kernel/wiki/principles/governance-approves-evidence-not-provenance.md)
