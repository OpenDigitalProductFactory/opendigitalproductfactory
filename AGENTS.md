...
- **Concurrent sessions:** one thread = one branch + one git worktree. Create with `git worktree add D:/DPF-worktrees/<topic> -b <prefix>/<topic>` (`~/dpf-worktrees/<topic>` on macOS/Linux). The dedicated sibling base `D:/DPF-worktrees/` is canonical for **both** host surfaces (Claude Code and Codex) per the 2026-06-05 unified-delivery-surfaces decision #1 — not Claude Code's default `.claude/worktrees/<random>` nesting and not the older `D:/DPF-<topic>` alongside-the-clone form. Never share a working tree across sessions; doing so causes index/HEAD collisions and cross-thread file sweeps. → [kernel principle](docs/founder-kernel/wiki/principles/worktree-per-session.md), [unified-delivery-surfaces §4.1](docs/superpowers/specs/2026-06-05-unified-delivery-surfaces-execution-alignment-design.md)
...
- **Keep the root clone as the merge/release worktree** — read-only for active feature work. Conventional locations: `d:\DPF` on Windows, `~/dpf` on macOS/Linux. Topic worktrees go in the dedicated sibling base `d:\DPF-worktrees\<topic>` (Windows) / `~/dpf-worktrees/<topic>` (macOS/Linux), canonical for both host surfaces per the 2026-06-05 unified-delivery-surfaces decision #1. → [kernel principle](docs/founder-kernel/wiki/principles/keep-root-clone-as-merge-worktree.md)
...
**Live portal refresh rule.** Normal feature verification and release validation MUST NOT rebuild the main `dpf` portal directly with `docker compose build`, `docker compose up`, `scripts/redeploy-portal.*`, or equivalent host-side image swaps. The main portal update path is `/ops/self-upgrade` (or the same governed self-upgrade runner/promoter path), which owns quiescence, recovery point creation, image rebuild/swap, health evidence, and rollback. Direct compose rebuilds of the main `dpf` `portal`/`portal-init` services are reserved for install/bootstrap/recovery work and must be named as such in the report; they do not count as the normal PR/release verification path. For unmerged feature work, use source-local gates plus the shared local-CI convergence sandbox or contributor preview instead of mutating the live portal.
...
- **DPF platform skills (both surfaces).** Catalogue entries show `name` — trigger; Surface-B `assignTo`; enforced kernel principles.
  ...
  - `dpf-ux-fit-review` — `UX fit|UI fit|design fit|feature fit|new route|new tab|first viewport|guided work|dashboard|cockpit|metric tile|KPI|status badge|card|coworker launcher|empty state|navigation change|portal UX`; `["ea-architect", "build-specialist", "platform-engineer"]`; `architecture-over-shortcuts`, `single-source-of-truth`, `no-hardcoded-colors`, `compose-report-kit-for-reporting-ux`, `design-research-required`. UX/IA fit gate before UI-impacting feature plans add routes, nav layers, dashboard bands, metric/status components, empty states, or coworker launch surfaces.
...
- **Three peer surfaces, one process.** DPF delivers through **Claude Code, Codex CLI, and the embedded Build Studio** — peers, not a hierarchy. None is privileged; work never depends on any one being healthy. All three advance the **one common evidence-gated lifecycle** (`ideate → plan → build → review → ship`), right-sized by `(type × size)`; each transition runs the gate check and nothing is rubber-stamped. → [kernel principle](docs/founder-kernel/wiki/principles/one-common-process-three-surfaces.md)
...
- **Governance approves evidence, not provenance — the keystone.** A gate reads only its required evidence fields and never branches on *which* surface produced them; that is exactly what makes the three surfaces interchangeable. → [kernel principle](docs/founder-kernel/wiki/principles/governance-approves-evidence-not-provenance.md)
- **Thin adapters behind a stable contract.** Claude Code and Codex ship updates *daily*; the surfaces are swappable adapters behind the gate/evidence + MCP + lease/worktree/sidecar contract. No surface-version-specific mechanics in the doctrine — if a rule would break when Claude or Codex bumps next week, it belongs in a thin adapter, not the contract.
...
- **`:3001` and every shared singleton are lease-gated.** Runtime-bound gates run only through `claim_nonprod_environment_lease(environmentKey="local-integration-ci")`; the `:3001` Contributor preview folds into that lease (§7 Q5). No per-branch CI images, no silent re-bind of a held shared singleton, no ad-hoc `docker run`/`compose up` from a surface. → [kernel principle](docs/founder-kernel/wiki/principles/runtime-gates-via-shared-lease.md)
  - **Enforced, not just documented (BI-4043A64B).** A `PreToolUse` guard (`scripts/hooks/lease-guard.mjs`, wired in `.claude/settings.json`) now *refuses* ungoverned dev-server launches (`pnpm/npm/yarn dev`, `next dev`, `turbo dev`, `dev-portal-start`) — this is what stops the rogue-server pileup, not goodwill. Governed path: `claim_nonprod_environment_lease` → run on the leased runtime, heartbeat with `renew_nonprod_environment_lease`, or run long-lived work in an isolated worktree compose stack; release on exit (the `SessionEnd` reaper also releases + kills sidecars). Emergency bypass: prefix `DPF_ALLOW_UNGATED_SERVER=1`. **Codex/Grok read this contract but get no `.claude/settings.json` hook — comply by construction (claim before you launch) until the shared `dpf worktree` wrapper lands (BI-6B02FEE5).** A lease can't be held indefinitely: the window is capped (`MAX_LEASE_TTL_MS`) and a holder that stops heartbeating is reaped, so no one process locks the single instance.
