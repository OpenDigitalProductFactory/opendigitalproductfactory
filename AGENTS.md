...
- **DPF platform skills (both surfaces).** Catalogue entries show `name` — trigger; Surface-B `assignTo`; enforced kernel principles.
  ...
  - `dpf-ux-fit-review` — `UX fit|UI fit|design fit|feature fit|new route|new tab|dashboard|cockpit|metric tile|KPI|status badge|card|coworker launcher|empty state|navigation change|portal UX`; `["ea-architect", "build-specialist", "platform-engineer"]`; `architecture-over-shortcuts`, `single-source-of-truth`, `no-hardcoded-colors`, `compose-report-kit-for-reporting-ux`, `design-research-required`. UX/IA fit gate before UI-impacting feature plans add routes, nav layers, dashboard bands, metric/status components, empty states, or coworker launch surfaces.
