...
- **Worktrees are source-control isolation, not runtime isolation.** A topic worktree exists to keep code changes off the root clone's HEAD; it is not a second DPF install. Implement and commit from the worktree, but perform functional validation against the canonical local install (the root clone's Docker stack) or a governed shared nonprod environment — unless the task is explicitly to harden the worktree harness itself. Do not spend the thread's budget making a worktree pnpm/Next/Prisma-runnable when the validation target is the canonical runtime. Harness friction (missing pnpm/corepack on PATH, workspace links pointing outside the worktree, missing Prisma client, Next/Turbopack rejecting cross-workspace symlinks) is a **harness limitation, not a product defect** — verify against the canonical install and file a separate platform BI if the worktree-runnability gap is worth closing. → [kernel principle](docs/founder-kernel/wiki/principles/worktree-is-source-control-not-runtime.md) — see also §5 (where each gate runs) and §6 (canonical-runtime evidence).
...
**Where each gate runs.** The four gates above describe *what* must pass, not *where* the binaries execute. Cheap source-local checks (targeted `vitest`, `pnpm --filter <pkg> typecheck`) can run in the topic worktree. Runtime-bound gates (`next build` against the production bundle, UX verification against the served portal, migration apply against a live Postgres) run against the **canonical local install** — the root clone's Docker stack or a leased shared nonprod environment — not a hand-built worktree harness (see §4 worktree bullet and the [kernel principle](docs/founder-kernel/wiki/principles/worktree-is-source-control-not-runtime.md)). A gate that did not execute because the worktree could not host its runtime is an **unrun gate, not a red gate**: re-run it against the canonical install, capture that evidence in the PR (§6), and reserve "make the worktree itself runnable" for a dedicated platform-process backlog item.
...
- **DPF platform skills (both surfaces).** Catalogue entries show `name` — trigger; Surface-B `assignTo`; enforced kernel principles.
  ...
  - `dpf-worktree-per-session` — `new worktree|create worktree|spawn session|concurrent session|parallel work|isolated branch`; `["build-specialist", "platform-engineer"]`; `worktree-per-session`, `propose-acknowledge-reassign`, `worktree-base-origin-main`, `keep-root-clone-as-merge-worktree`, `worktree-is-source-control-not-runtime`.
