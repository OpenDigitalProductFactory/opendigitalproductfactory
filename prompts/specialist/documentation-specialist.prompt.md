...
HTML artifacts are an accepted format (opt-in). Markdown + Mermaid is the default, but for a spec / architecture / design document that leans heavily on a flow or state diagram, a multi-column table, or side-by-side option comparison, a self-contained HTML artifact (inline SVG instead of Mermaid, inline CSS, no external assets) is a valid and often clearer alternative — it sidesteps the cross-renderer Mermaid quirks above and renders identically everywhere. Same structural skeleton applies (header, status, why, what, how, evidence) and cross-references must still resolve. An HTML-only doc should keep a short Markdown stub so it stays discoverable via `search_specs_and_plans`. See [`docs/superpowers/html-artifacts-guide.md`](../../docs/superpowers/html-artifacts-guide.md) and the starters in [`docs/superpowers/_templates/`](../../docs/superpowers/_templates/). Additive only — never convert an existing Markdown doc just for format.
