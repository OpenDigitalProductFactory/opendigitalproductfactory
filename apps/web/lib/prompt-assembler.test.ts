...
describe("assembleSystemPrompt", () => {
  ...
  it("places all blocks in correct order: identity → authority → mode → sensitivity → domain → route → attachments", () => {
  ...
  it("injects advise mode text when mode is 'advise'", () => {
  ...
  it("injects act mode text when mode is 'act'", () => {
    const prompt = assembleSystemPrompt(fullInput);
  ...
  it("lists granted and denied capabilities in the authority block", () => {
    const prompt = assembleSystemPrompt(fullInput);
  ...
  it("includes sensitivity level capitalized in the sensitivity block", () => {
    const prompt = assembleSystemPrompt(fullInput);
    ...
    const restrictedPrompt = assembleSystemPrompt(restrictedInput);
    ...
    const confidentialPrompt = assembleSystemPrompt(confidentialInput);
    expect(confidentialPrompt).toContain("classified CONFIDENTIAL");
  });

  // Test 6: Domain tools are listed
  it("lists domain tools in the domain context block", () => {
    const prompt = assembleSystemPrompt(fullInput);
  ...
  it("includes PLATFORM_PREAMBLE behavioral rules in the identity block", () => {
    const prompt = assembleSystemPrompt(fullInput);
  ...
  it("omits route data block when routeData is null", () => {
    const prompt = assembleSystemPrompt(minimalInput);
  ...
  it("includes route data block when routeData is present", () => {
    const prompt = assembleSystemPrompt(fullInput);
  ...
  it("includes attachment block when attachmentContext is present", () => {
    const prompt = assembleSystemPrompt(fullInput);
  ...
  it("omits attachment block when attachmentContext is null", () => {
    const prompt = assembleSystemPrompt(minimalInput);
  ...
  it("omits domain tools line when domainTools is empty", () => {
    const prompt = assembleSystemPrompt(minimalInput);
  ...
  it("handles empty denied capabilities list", () => {
    const prompt = assembleSystemPrompt(minimalInput);
