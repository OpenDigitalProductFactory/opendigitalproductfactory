...
describe("routeEndpointV2", () => {
  ...
  it("does not let a persisted pin override unhealthy provider capacity", async () => {
    const decision = await routeEndpointV2(
      [qualityModel, cheapModel],
      makeContract(),
      [],
    ...
    expect(decision.selectedEndpoint).toBe("ep-quality");
    expect(
      decision.candidates.find((candidate) => candidate.endpointId === "ep-cheap"),
    ).toMatchObject({
      excluded: true,
    ...
    expect(decision.preferenceResolution).toMatchObject({
      fallbackUsed: true,
      unavailable: [{ kind: "endpoint", value: "ep-cheap" }],
    });
  });
  ...
  it("builds the execution plan for the finalized provider preference", async () => {
    const decision = await routeEndpointV2(
      [qualityModel, cheapModel],
  ...
  it("does not let a persisted pin bypass local-only residency", async () => {
    const decision = await routeEndpointV2(
      [cheapModel, localModel],
      makeContract({ residencyPolicy: "local_only" }),
      [],
      [
        {
          endpointId: "ep-cheap",
    ...
    expect(
      ...
      excluded: true,
      excludedReason: expect.stringContaining("local_only"),
    });
  });

  it("does not let a persisted pin bypass sensitivity clearance", async () => {
    const decision = await routeEndpointV2(
      [cheapModel, localModel],
    ...
    expect(
      ...
      excluded: true,
      excludedReason: expect.stringContaining("Sensitivity clearance"),
    });
  });

  it("does not let a persisted pin bypass an agent capability floor", async () => {
    const capableModel = makeEndpoint({
      ...qualityModel,
      id: "ep-capable",
      capabilities: {
    ...
    const decision = await routeEndpointV2(
      [cheapModel, capableModel],
      makeContract({ minimumCapabilities: { codeExecution: true } }),
      [],
      [
        {
          endpointId: "ep-cheap",
  ...
  it("does not let a persisted pin bypass the context-window floor", async () => {
    const pinnedSmallModel = makeEndpoint({
      ...cheapModel,
      maxContextTokens: 8_192,
    });
    const decision = await routeEndpointV2(
      [pinnedSmallModel, qualityModel],
      makeContract({ minContextTokens: 32_000 }),
      [],
      [
        {
          endpointId: "ep-cheap",
    ...
    expect(
      ...
      excluded: true,
      excludedReason: expect.stringContaining("Context window"),
    });
  });

  it("does not let a persisted pin bypass minimum quality dimensions", async () => {
    const decision = await routeEndpointV2(
      [cheapModel, qualityModel],
      makeContract({ minimumDimensions: { reasoning: 80 } }),
      [],
      [
        {
          endpointId: "ep-cheap",
    ...
    expect(
      ...
      excluded: true,
      excludedReason: "Minimum quality dimensions not met",
    });
  });
