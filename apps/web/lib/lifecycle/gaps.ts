...
export function buildGapKey(parts: {
  ...
  return parts.signalId ? `${base}:${parts.signalId}` : base;
}
...
export type GapPlanSummary = { created: number; updated: number; resolved: number; unchanged: number };
 ...
 */
export function reconcileGaps(generated: LifecycleGapInput[], existingOpen: ExistingGap[]): GapPlan {
  const existingByKey = new Map<string, ExistingGap>();
  for (const g of existingOpen) existingByKey.set(g.gapKey, g);

  const ops: GapOp[] = [];
  const summary: GapPlanSummary = { created: 0, updated: 0, resolved: 0, unchanged: 0 };
