...
const definitions: ToolDefinition[] = [
  ...
  {
    name: "adopt_worktree",
    description: "Adopt an existing local branch/worktree pair into a Workroom without creating a new worktree. Pass backlogItemId to bind the work: the returned capsule carries the binding or the call fails, never a partially-populated capsule. A branch has one durable workroom identity; a terminal capsule with no backlog item is resumed and rebound rather than blocking the branch, while a foreign binding returns branch_occupied instead of overwriting history.",
    inputSchema: {
      type: "object",
      properties: {
        title: { type: "string", description: "Short workroom title." },
        ...
        backlogItemId: { type: "string", description: "BacklogItem this work delivers (BI-*). Bound onto the capsule and read back; an unknown id is refused rather than dropped." },
