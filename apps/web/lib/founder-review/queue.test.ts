...
// BI-F5F2869D — an approve-direction escalation is a NEW IDEA to weigh, not a
// doctrine gap to fill. Labelling it "principle gap → clarify operating policy"
// told the operator to write doctrine when what they actually had was a novel
// proposition their existing doctrine already permits.
describe("a new proposition is labelled as one, not as a gap (BI-F5F2869D)", () => {
  function orgRow(payload: Record<string, unknown>): DecisionInteractionQueueRow {
    return {
      interactionId: "DI-NEW",
      question: "Should we onboard an MSP reseller partner?",
      options: [],
      outcomeType: "escalate",
      outcomePayload: payload,
      buildId: null,
  ...
  it("reports an aligned-but-unruled decision as a new proposition", () => {
    const c = projectFounderReviewCandidate(orgRow({
      coverageGap: false,
      stanceAlignment: "approve",
    ...
    expect(c.unresolvedReason).toBe("aligned-not-settled");
    expect(c.unresolvedReasonLabel).toBe("New proposition");
    expect(c.primaryActionLabel).toBe("Weigh this new proposition");
  });

  it("does not call it a principle gap or ask for policy", () => {
    const c = projectFounderReviewCandidate(orgRow({
      coverageGap: false,
      stanceAlignment: "approve",
      confidenceScore: 0.9,
    }));
    expect(c.unresolvedReason).not.toBe("principle-gap");
    expect(c.primaryActionLabel).not.toBe("Clarify operating policy");
  });

  // A genuine absence of doctrine still reads as a gap — widening the new-idea
  // label to cover real gaps would hide the case that DOES need policy.
  it("still reports a real coverage gap as a principle gap", () => {
    const c = projectFounderReviewCandidate(orgRow({ coverageGap: true, stanceAlignment: "approve" }));
    expect(c.unresolvedReason).toBe("principle-gap");
  });

  it("still reports a conflict as a conflict", () => {
    const c = projectFounderReviewCandidate(orgRow({
      coverageGap: false,
      stanceAlignment: "mixed",
  ...
  it("still reports a degraded embedding layer rather than a new idea", () => {
    const c = projectFounderReviewCandidate(orgRow({
      coverageGap: false,
      stanceAlignment: "approve",
      relevanceMethod: "lexical",
