...
describe("generated route-shell registry", () => {
  ...
  it("accounts for every route as measurable or explicitly excluded", () => {
    ...
    // The three time/live-state exclusions above plus the two new dynamic
    // catalog workspaces are all explicit in ROUTE_SWEEP_EXCLUSIONS.
    expect(registry.routes.filter((route) => !route.sweepEligible)).toHaveLength(115);
  });
