...
// The backlog write no longer happens here — createBacklogItemFromFinding files
// through the shared ingestBacklogItem front door (EP-INTAKE-UNIFY). This db
// surface only reads/updates the finding and resolves/reopens the epic.
export type CreateBacklogFromFindingDb = FindingActionsDb & EpicLookupDb;
...
export async function createBacklogItemFromFinding(
  ...
  ingest: (input: BacklogIngestInput) => Promise<BacklogIngestResult> = ingestBacklogItem,
  ...
  // Converge onto the shared backlog-ingest front door (EP-INTAKE-UNIFY): file
  // through the same path every detector uses, so the BI carries a typed origin
  // marker + an intake_origin activity row instead of a bespoke create. The
  // finding stays the evidence record; the BacklogItem is the work.
  const item = await ingest({
    ...
    origin: { kind: "assuranceFinding", id: existing.findingKey },
  });
