...
export type ResolvedPostureSource = "workroom" | "organization" | "platform";
...
// ─── Per-coworker posture: RETIRED (BI-7ADEBDC1, founder direction 2026-09-08) ──
//
// The Golden Triangle is a parameter of the Workroom definition, chosen for the
// room's expected outcome; it is not a coworker's identity preference. The
// `goldenTrianglePerAgent` map that used to live under the platform profile's
// autonomyPolicy is left in place and INERT — an identity preference cannot be
// reinterpreted as an outcome preference without inventing a choice the owner
// never made, so it is ignored rather than migrated (same reasoning as
// BI-87C9C91C for proactivity). Room priority reaches dispatch through
// RouteAndCallOptions.workroomPriority, resolved by
// lib/work-management/room-turn-authority.ts.
...
export async function getEffectivePostureForAgent(
  ...
  // The agent layer is retired (see above); a coworker inherits the room it
  // works in, then the org, then the platform. Kept as the callers' entry
  // point so the identity-to-scope seam has one name.
