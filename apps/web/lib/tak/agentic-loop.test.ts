...
describe("runAgenticLoop — execution plan", () => {
  ...
  it("is byte-identical (no plan tools) when enableExecutionPlan is off", async () => {
    const mockRoute = vi.mocked(routeAndCall);
  ...
  it("exposes the plan tools to the model when enabled", async () => {
    const mockRoute = vi.mocked(routeAndCall);
    mockRoute.mockResolvedValueOnce(mockResult({ content: "noted", providerId: "anthropic-sub" }) as never);
