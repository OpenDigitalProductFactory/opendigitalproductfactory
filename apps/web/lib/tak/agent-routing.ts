...
ON THIS PAGE: The user sees the Build Studio with conversation panel, feature brief/preview, and phase indicator.`,
    skills: [
      ...
      { label: "Review UI quality", description: "Audit the current build for design system compliance and accessibility", capability: "view_platform", prompt: "Audit the UI code in this build for quality. Use list_sandbox_files to find all .tsx component files, then read_sandbox_file on each. Check for: (1) hardcoded hex colors — must use var(--dpf-*) tokens, (2) missing aria-labels on buttons/inputs, (3) <span role='button'> instead of <button>, (4) no loading/skeleton states for async operations, (5) missing focus-visible rings on interactive elements, (6) text using var(--dpf-muted) for body content (should be var(--dpf-text-secondary) for 4.5:1 contrast), (7) fixed widths without responsive breakpoints. Report each issue with file:line and a one-line fix. Then ask if I want you to fix them." },
      { label: "Polish interactions", description: "Add animations, hover states, and micro-interactions", capability: "view_platform", prompt: "Polish the UI interactions in this build. Use list_sandbox_files and read_sandbox_file to find components, then enhance: (1) add hover states — hover:bg-[var(--dpf-surface-2)] and hover:shadow-dpf-xs on clickable cards, (2) add entrance animations — animate-slide-up with staggered delays on list items, animate-fade-in on content panels, animate-scale-in on modals, (3) add transition-colors/transition-all on interactive elements, (4) add disabled states — disabled:opacity-50 disabled:cursor-not-allowed, (5) add loading spinners on async buttons. Use edit_sandbox_file for each change." },
      { label: "Make responsive", description: "Add mobile and tablet support with Tailwind breakpoints", capability: "view_platform", prompt: "Make this build's UI responsive. Use list_sandbox_files and read_sandbox_file to find layout components, then fix: (1) fixed-width sidebars need w-full lg:w-[360px] with a collapse toggle, (2) grid layouts need grid-cols-1 md:grid-cols-2 lg:grid-cols-3, (3) text sizes need responsive scaling — text-sm lg:text-base, (4) touch targets must be minimum 44px on mobile, (5) tables need horizontal scroll wrappers on small screens, (6) modals/panels need max-w-[calc(100vw-2rem)] on mobile. Apply fixes with edit_sandbox_file." },
