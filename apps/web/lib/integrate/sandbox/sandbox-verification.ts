...
export function sandboxTypecheckCommand(workdir: string = VERIFY_WORKSPACE_DEFAULT): string {
  return `cd ${workdir} && NODE_OPTIONS=--max-old-space-size=24576 pnpm --filter web typecheck 2>&1`;
}
...
export function sandboxBuildCommand(workdir: string = VERIFY_WORKSPACE_DEFAULT): string {
  return `cd ${workdir} && NODE_ENV=production NODE_OPTIONS=--max-old-space-size=24576 pnpm --filter web build 2>&1`;
}
