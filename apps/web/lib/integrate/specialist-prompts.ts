...
Elevation:
- Elevation tokens are not yet defined (they land with the EP-UX-SYSTEM L0 token scales). Use standard Tailwind shadows sparingly (shadow-sm, shadow-lg). Do NOT write shadow-dpf-* classes — they are not generated by this Tailwind v4 setup and silently no-op.

Motion:
- Loading/entrance motion is owned by the components/ui primitives (Spinner, Skeleton, InlineBusy); prefers-reduced-motion is handled in globals.css.
- Use transition-colors (150–300ms) for hover/focus feedback.
- Do NOT write animate-fade-in / animate-slide-up / animate-scale-in — these classes are not generated by this Tailwind v4 setup and silently no-op.
