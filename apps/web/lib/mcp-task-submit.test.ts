...
describe("submitRemoteCoworkerTask idempotency", () => {
  ...
  it("recovers the same stalled review TaskRun into fresh exact approval without rerunning inference", async () => {
    ...
    const bindingFingerprint = fingerprintCoworkerApprovalBinding(approvalBinding);
    const run = {
      id: "task-internal",
      taskRunId: approvalBinding.taskRunId,
      userId: "user-1",
      threadId: "thread-bi47",
      contextId: "thread-bi47",
      status: "stalled",
      ...
      completedAt: new Date("2026-08-28T13:45:27.398Z"),
      updatedAt,
    };
    ...
    db.findEnvelope.mockResolvedValue({
      id: "ENV-EXPIRED",
      coworkerAgentId: params.agentId,
      delegatingUserId: "user-1",
      threadId: "thread-bi47",
      chatMessageId: null,
      manifestActionId: approvalBinding.toolName,
      argsJson: { approvalBinding },
      ...
      approvalBindingFingerprint: bindingFingerprint,
    ...
    db.findToolExecution
      ...
      .mockResolvedValueOnce({
        ...
        agentId: params.agentId,
        userId: "user-1",
        toolName: approvalBinding.toolName,
        parameters: { decision: "pass", backlogItemId: "BI-47ACE2C7" },
        result: { data: { envelopeId: "ENV-EXPIRED" } },
        success: false,
    ...
    const outcome = await submit("PAT-BI47", params);
