...
describe("WordPress connection operations", () => {
  it("checks health with the stored credential and records only safe evidence", async () => {
    ...
    const result = await checkWordPressConnection({
      ...
      createClient: () => ({ probe: vi.fn(async () => ({ siteName: "Acme", origin: "https://wordpress.example", authenticatedUser: { id: 7, name: "Publisher" }, supportedResourceKinds: ["post", "page"] as Array<"post" | "page" | "media">, canCreateDrafts: true, canPublishLive: false, canUploadMedia: false })) }),
      now: () => new Date("2026-08-22T06:00:00.000Z"),
    });
    expect(result).toMatchObject({ ok: true, siteName: "Acme", canPublishLive: false });
    expect(recordHealthProbe).toHaveBeenCalledWith("wordpress-self-hosted", { succeeded: true });
