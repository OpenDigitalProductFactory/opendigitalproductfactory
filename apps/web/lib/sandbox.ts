...
export async function startSandboxDevServer(containerId: string): Promise<void> {
  ...
  // Preview server: serves /workspace/_preview/index.html (visual mockup)
  // generated by the AI alongside the actual code. Falls back to a
  // "building..." spinner if no preview exists yet.
  const previewScript = `
