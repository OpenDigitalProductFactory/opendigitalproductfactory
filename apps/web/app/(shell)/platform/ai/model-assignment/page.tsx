...
export default async function ModelAssignmentPage() {
  ...
  const [dbConfigs, providers, lastModels] = await Promise.all([
  ...
  const providerList = providers.map((p) => ({
    ...
    name: p.name,
    models: p.modelProfiles.map((m: { modelId: string; friendlyName: string }) => ({
      modelId: m.modelId,
