...
describe("AgentDetailPage", () => {
  ...
  it("does not offer Ask Customer Advisor for the canonical service while its declaration is incomplete", async () => {
    vi.mocked(prisma.agent.findFirst).mockReset();
    vi.mocked(prisma.agent.findFirst)
      .mockResolvedValueOnce({
        id: "agent-db-customer-canonical",
    ...
    vi.mocked(prisma.coworkerService.findMany).mockResolvedValue([
      {
        ...
        summary:
          "External-facing customer inquiry, quote, support, and service-request triage.",
        status: "active",
        hitlTier: 1,
        authorityBoundary: "proposal-only",
        availabilityScope: "external",
