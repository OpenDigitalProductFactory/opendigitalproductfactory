...
export default async function IdentityAuthorizationBindingPage({ params }: Props) {
    ...
    <div className="space-y-4">
      <div>
        <div className="text-xs uppercase tracking-[0.08em] text-[var(--dpf-muted)]">
          <Link href="/platform/identity/authorization" className="text-[var(--dpf-accent)]">
            Authorization bindings
          </Link>
          {" · "}Identity &amp; Access{" · "}Authorization
        </div>
        <p className="mt-2 text-sm text-[var(--dpf-muted)]">
          Full-page fallback for the shared authority binding detail surface.
        </p>
      </div>
