...
export default async function EvidenceDetailPage({ params }: Props) {
  ...
  return (
    <div>
      {/* Breadcrumb */}
      <div className="mb-2">
        <Link href="/compliance/evidence" className="text-xs text-[var(--dpf-muted)] hover:text-white">Evidence</Link>
        <span className="text-xs text-[var(--dpf-muted)]"> / </span>
        <span className="text-xs text-white">{evidence.title}</span>
      </div>

      {/* Header */}
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          <h1 className="text-xl font-bold text-white">{evidence.title}</h1>
      ...
      {evidence.status === "superseded" && evidence.supersededBy && (
        <div className="mb-6 p-3 rounded-lg border border-yellow-500/50 bg-yellow-900/10">
          ...
          >
            ...
            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#333] text-[var(--dpf-muted)]">{evidence.supersededBy.evidenceId}</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div className="p-3 rounded-lg border border-[var(--dpf-border)]">
          ...
          <p className="text-sm font-semibold text-white">{new Date(evidence.collectedAt).toLocaleString()}</p>
        </div>
        {evidence.collectedBy && (
          <div className="p-3 rounded-lg border border-[var(--dpf-border)]">
            <p className="text-xs text-[var(--dpf-muted)]">Collected By</p>
            <p className="text-sm font-semibold text-white">{evidence.collectedBy.displayName}</p>
          </div>
        )}
        {evidence.fileRef && (
          <div className="p-3 rounded-lg border border-[var(--dpf-border)]">
            <p className="text-xs text-[var(--dpf-muted)]">File Reference</p>
            <p className="text-sm font-semibold text-white break-all">{evidence.fileRef}</p>
          </div>
        )}
        {evidence.retentionUntil && (
          <div className="p-3 rounded-lg border border-[var(--dpf-border)]">
            <p className="text-xs text-[var(--dpf-muted)]">Retain Until</p>
            <p className="text-sm font-semibold text-white">{new Date(evidence.retentionUntil).toLocaleDateString()}</p>
          </div>
        )}
      </div>
      ...
      {evidence.obligation && (
        <div className="mb-6">
          ...
          >
            <span className="text-sm font-semibold text-white">{evidence.obligation.title}</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#333] text-[var(--dpf-muted)]">{evidence.obligation.obligationId}</span>
      ...
      {evidence.control && (
        <div className="mb-6">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 p-3 rounded-lg border border-[var(--dpf-border)] hover:border-[var(--dpf-accent)] transition-colors"
          >
            <span className="text-sm font-semibold text-white">{evidence.control.title}</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#333] text-[var(--dpf-muted)]">{evidence.control.controlId}</span>
            <span className="text-xs text-blue-400">View</span>
          </Link>
        </div>
      )}
