...
const { mockAuth, mockConnectFacebookPages } = vi.hoisted(() => ({
  mockAuth: vi.fn(),
  mockConnectFacebookPages: vi.fn(),
}));
...
function makeReq(body: unknown, options: { raw?: string } = {}): Request {
  ...
  return new Request("http://test/api/integrations/facebook-pages/connect", {
    method: "POST",
...
beforeEach(() => {
  mockAuth.mockReset();
  mockConnectFacebookPages.mockReset();
});

describe("POST /api/integrations/facebook-pages/connect", () => {
  it("returns 401 when no session", async () => {
    mockAuth.mockResolvedValue(null);
    const res = await POST(makeReq({}));
    expect(res.status).toBe(401);
    expect(mockConnectFacebookPages).not.toHaveBeenCalled();
  });
  ...
  it("returns 200 on success", async () => {
    ...
    mockConnectFacebookPages.mockResolvedValue({
      ok: true,
      status: "connected",
      pageId: "123456789",
