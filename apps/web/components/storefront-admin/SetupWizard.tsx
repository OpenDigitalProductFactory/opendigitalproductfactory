    ...
    return (
      <div>
        ...
        <input
          ...
          style={{ width: "100%", maxWidth: 360, padding: "8px 12px", borderRadius: 6, border: "1px solid var(--dpf-border)", fontSize: 14, marginBottom: 16 }}
        />
        {categories.map((cat) => {
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 8 }}>
                {items.map((a) => (
                  <button key={a.archetypeId} onClick={() => { setSelected(a); setStep(2); }}
                    style={{ padding: "12px 16px", textAlign: "left", borderRadius: 8, border: "1px solid var(--dpf-border)", background: "none", cursor: "pointer", fontSize: 13 }}>
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setStep(1)} style={{ padding: "8px 16px", borderRadius: 6, border: "1px solid var(--dpf-border)", background: "none", cursor: "pointer", fontSize: 13 }}>Back</button>
    ...
    <div style={{ maxWidth: 480 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <label style={{ fontSize: 13 }}>
          ...
          <input type="text" value={orgName} onChange={(e) => { setOrgName(e.target.value); setOrgSlug(derivedSlug(e.target.value)); }}
            required style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid var(--dpf-border)", fontSize: 14 }} />
        </label>
        <label style={{ fontSize: 13 }}>
          <div style={{ fontWeight: 600, marginBottom: 4 }}>URL slug *</div>
          <input type="text" value={orgSlug} onChange={(e) => setOrgSlug(e.target.value)}
            required style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid var(--dpf-border)", fontSize: 14, fontFamily: "monospace" }} />
          <div style={{ fontSize: 11, color: "var(--dpf-muted)", marginTop: 4 }}>Your storefront will be at /s/{orgSlug || "your-slug"}</div>
        </label>
        <label style={{ fontSize: 13 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setStep(2)} style={{ padding: "8px 16px", borderRadius: 6, border: "1px solid var(--dpf-border)", background: "none", cursor: "pointer", fontSize: 13 }}>Back</button>
