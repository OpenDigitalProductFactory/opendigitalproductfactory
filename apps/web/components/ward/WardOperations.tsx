    ...
    <section className="space-y-4" aria-labelledby="ward-actions-heading">
          ...
          <div className="flex flex-wrap gap-2 pb-2" aria-label="Current stays">
            {placed.map((animal) => (
              <form action={action} key={animal.animalRef}>
                ...
                <SubmitButton pending={pending} pendingLabel={`Releasing ${animal.name}…`}>
                  Release {animal.name}
                </SubmitButton>
              </form>
            ))}
      ...
      <details className="rounded-lg border border-[var(--dpf-border)] p-4">
        ...
        <form action={action} className="mt-3 grid gap-3 md:grid-cols-2">
          ...
          <TextField name="label" label="Label" required value={label} onValueChange={setLabel} maxLength={120} placeholder="D4 or Northside foster" />
          <SelectField
            name="kindSlug"
            label="Kind"
            required
          ...
          />
          <TextField name="serviceArea" label="Area" optional value={serviceArea} onValueChange={setServiceArea} maxLength={120} placeholder="Dog ward or Foster network" />
          <TextField name="capacity" label="Animal capacity" type="number" required value={capacity} onValueChange={setCapacity} min={1} max={kindSlug === "foster-home" ? 12 : 100} />
          <div className="md:col-span-2">
            <SubmitButton pending={pending} pendingLabel="Adding housing…">Add housing</SubmitButton>
          </div>
        </form>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <form action={action}>
                    ...
                    <ConsequenceNotice
                      ...
                      tone="warning"
                    />
                    <SubmitButton pending={pending} pendingLabel="Retiring…">Retire</SubmitButton>
                  </form>
                </div>
                </div>
