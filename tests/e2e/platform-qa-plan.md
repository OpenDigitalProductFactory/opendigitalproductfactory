...
| AI-17 | **(External Access permission)** On `/finance/settings/tax`, keep External Access Off and ask the Finance Specialist to research current official public guidance; then enable External Access from the coworker header | Coworker asks the user to enable External Access instead of pretending to search, names the public sources or search targets it needs, records an `external_access_permission_request` audit row, preserves the same thread/task context after enablement, continues the prior request, and records a matching `external_access_permission_approval` audit row. |
