...
| SETUP-06 | Fresh install rehearsal with empty DPF runtime volumes | First visit redirects to `/setup`; no warmed-up organization, provider credential, backlog, or session state is silently reused |
...
| BUILD-46 | Run a sandboxed Build Studio task that can use Codex CLI and Claude Code CLI, or run it in an environment where one is missing | The capsule records distinct executor session/evidence entries for each available CLI; missing CLI/provider/OAuth setup appears as a blocked provider requirement, not as a false green |
