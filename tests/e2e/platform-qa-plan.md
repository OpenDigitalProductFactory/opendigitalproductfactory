...
| FIN-10 | In `/finance/settings/tax`, save authority credential custody for a verified registration, generate a tax execution run, then record a submitted or failed outcome | Credential custody saves without exposing plaintext secrets, the execution run appears on the page, liability lineage remains visible, and outcome updates create the expected operational state without adding coworker dialog to the page |
...
| BUILD-44 | After login, open `/build/work` with multiple local worktrees present | Work Control renders active capsules and adoptable work; the release `main` worktree is not offered as adoptable implementation work |
| BUILD-45 | Run a capsule-managed change through branch push evidence before merge readiness | Branch is pushed and linked to the Work Capsule, but no PR is opened until focused tests, production build, migration evidence if needed, and UX evidence are present |
| BUILD-46 | Run a sandboxed Build Studio task that can use Codex CLI, Claude Code CLI, or Grok CLI (or run the scratch rehearsal plan in an environment where one or more are missing) | The capsule records distinct executor session/evidence entries for each available CLI (claude/codex/grok dispatch paths); missing CLI/provider/OAuth setup appears as a blocked provider requirement, not as a false green. All three external CLIs must demonstrate functional parity for specialist task dispatch (auth injection, prompt construction with DPF monorepo discipline, realtime progress, audit, timeouts, result shaping). |
| BUILD-47 | Merge-ready capsule reaches PR creation gate | PR body includes `DPF-Capsule: <capsuleId>` and verification evidence; opening the PR is treated as the ready-to-merge signal |
...
| AI-17 | **(External Access permission)** On `/finance/settings/tax`, keep the coworker's Web access toggle off (in the header Controls menu) and ask the Finance Specialist to research current official public guidance; then enable Web access from the Controls menu | Coworker asks the user to enable External Access instead of pretending to search, names the public sources or search targets it needs, records an `external_access_permission_request` audit row, preserves the same thread/task context after enablement, continues the prior request, and records a matching `external_access_permission_approval` audit row. |
