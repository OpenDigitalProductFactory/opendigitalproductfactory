...
if (-not (Is-StepDone "download")) {
    ...
    if (Test-Path "$DPF_DIR\docker-compose.yml") {
        ...
        if ($modeChoice -eq "2") {
            ...
            # --- Developer environment setup (mirrors scripts\fresh-install.ps1) ---

            # Node.js is required for local development (pnpm dev, prisma, etc.)
            ...
            # Create app-level .env files needed for local Next.js and Prisma
            ...
            # Create docker-compose.override.yml: expose DB ports + store volumes on project drive
            $overridePath = "$DPF_DIR\docker-compose.override.yml"
            if (-not (Test-Path $overridePath)) {
                $installDriveLetter = (Split-Path -Qualifier $DPF_DIR).TrimEnd(':')
                $dockerDataDir = "${installDriveLetter}:\docker-data\dpf"
                foreach ($subdir in @("pgdata", "neo4jdata", "qdrant_data")) {
                    $p = Join-Path $dockerDataDir $subdir
                    if (-not (Test-Path $p)) { New-Item -ItemType Directory -Path $p -Force | Out-Null }
                }
                $dockerDataSlash = $dockerDataDir.Replace('\', '/')
@"
# Auto-generated by DPF installer (customizer mode)
# Stores Docker volumes on ${installDriveLetter}: drive and exposes DB ports for local development.
services:
  postgres:
    ports:
      - "5432:5432"
    volumes:
      - "${dockerDataSlash}/pgdata:/var/lib/postgresql/data"
  neo4j:
    ports:
      - "7687:7687"
      - "7474:7474"
    volumes:
      - "${dockerDataSlash}/neo4jdata:/data"
  qdrant:
    ports:
      - "6333:6333"
    volumes:
      - "${dockerDataSlash}/qdrant_data:/qdrant/storage"
"@ | Set-Content -Path $overridePath -Encoding UTF8
                Write-OK "Created docker-compose.override.yml (volumes on ${installDriveLetter}:, ports exposed for pnpm dev)"
            }
