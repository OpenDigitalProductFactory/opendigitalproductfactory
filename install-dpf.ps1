...
if ($InstallMode -eq "customizer" -and -not (Is-StepDone "dev_setup")) {
    ...
    # docker-compose.override.yml: expose DB ports + store volumes on project drive
...
"@ | Set-Content -Path $overridePath -Encoding UTF8
        Write-OK "Created docker-compose.override.yml (volumes on ${installDriveLetter}:, ports exposed)"
    }
